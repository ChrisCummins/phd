...
class MirroredDirectory(object):
  ...
  def __init__(
    self, host: machine_spec_pb2.Host, spec: machine_spec_pb2.MirroredDirectory
  ):
    self.host = host
    self.spec = spec
  ...
  def PushFromLocalToRemote(
    ...
    if self.skip_if_not_present and not self.LocalExists():
      app.Log(
        1, "Skipping local path that does not exist: `%s`", self.local_path
      )
      return
  ...
  def __repr__(self) -> str:
    return (
      f"MirroredDirectory(name={self.name}, "
  ...
  def _Ssh(self, cmd: str) -> str:
    ...
    return subprocess.check_output(
      ["ssh", "-p", str(self.host.port), self.host.host, "-t", cmd],
