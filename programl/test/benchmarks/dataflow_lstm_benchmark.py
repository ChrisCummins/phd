...
"""Benchmark for the dataflow experiment pipeline."""
import contextlib
import os
import pathlib
import sys
...
app.DEFINE_integer(
  "train_batch_count", 20, "The number of batches for testing model training"
)
app.DEFINE_integer(
  "test_batch_count", 40, "The number of batches for testing model training"
)
FLAGS = app.FLAGS
...
def CreateWarmUpBatch(path):
  def Wrapped(model):
    return list(
      DataflowLstmBatchBuilder(
        graph_loader=DataflowGraphLoader(
          path=path, epoch_type=epoch_pb2.TRAIN, analysis="reachability",
        ),
...
def Main():
  ...
  with data_directory() as path:
    ...
    model = Lstm(
      ...
      create_warm_up_batch=CreateWarmUpBatch(path),
    )
