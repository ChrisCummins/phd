...
from programl.ml.model import logger as logger_lib
...
def SplitsFromFlags(
  ...
  return {
    epoch.EpochType.TRAIN: train_splits,
    epoch.EpochType.VAL: val_splits,
    epoch.EpochType.TEST: test_splits,
  }
...
def RunEpoch(
  ...
  def GetEpochLabel(results: epoch.EpochResults) -> str:
...
class TrainValTestLoop(progress.Progress):
  ...
  def MakeBatchIterator(
    self, epoch_type: epoch.EpochType
  ...
  def RunOneEpoch(self, test_on: str, save_on) -> epoch.EpochResults:
    ...
    train_batches = self.MakeBatchIterator(epoch.EpochType.TRAIN)
    val_batches = self.MakeBatchIterator(epoch.EpochType.VAL)
    if test_on == "every":
      # If we know that we're going to use them, produce the test batches.
      test_batches = self.MakeBatchIterator(epoch.EpochType.TEST)

    # Run the training and validation epochs.
    train_results, _ = self.RunEpoch(epoch.EpochType.TRAIN, train_batches)
    val_results, val_improved = self.RunEpoch(epoch.EpochType.VAL, val_batches)
    ...
    if test_on == "improvement" and self.ctx.i == 0:
      ...
      self.RunEpoch(
        epoch.EpochType.TEST, self.MakeBatchIterator(epoch.EpochType.TEST)
      )
    ...
    if test_on == "every":
      self.RunEpoch(epoch.EpochType.TEST, test_batches)

    return val_results

  def ShouldExitEarly(self, val_results: epoch.EpochResults) -> bool:
    ...
    if self.patience and (
      (self.ctx.i - self.model.best_results[epoch.EpochType.VAL].epoch_num)
      >= self.patience
    ):
      app.Log(
        1,
  ...
  def RunEpoch(
    self, epoch_type: epoch.EpochType, batch_iterator: batchs.BatchIterator,
  ) -> Tuple[epoch.EpochResults, int]:
