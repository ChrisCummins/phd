...
from programl.ml.model.ggnn.ggnn_config import GGNNConfig
from programl.ml.model.ggnn.ggnn_modules import GGNNModel
...
class Ggnn(Model):
  ...
  def __init__(
    ...
    # Instantiate model
    config = GGNNConfig(
      num_classes=(self.node_y_dimensionality or self.graph_y_dimensionality),
      has_graph_labels=self.graph_y_dimensionality > 0,
      has_aux_input=self.graph_x_dimensionality,
    )

    inst2vec_embeddings = inst2vec_encoder.Inst2vecEncoder().embeddings_tables[
      0
    ]
  ...
  def get_unroll_steps(
    ...
    if unroll_strategy == "none":
      ...
      max_edge_count = max(
        graph.edge_count for graph in batch.model_data.graphs
      )
  ...
  def PrepareModelInputs(
    ...
    """
    graph_tuple: GraphTuple = batch.model_data
    ...
    # TODO: vocab ids
    vocab_ids = torch.from_numpy(np.zeros(len(graph_tuple.features))).to(
      self.model.dev, torch.long
    )
    selector_ids = torch.from_numpy(graph_tuple.features[:, 0]).to(
      self.model.dev, torch.long
    )
    # we need those as a result on cpu and can save device i/o
    # Expand to 1-hot.
    labels = graph_tuple.labels
  ...
  def RunBatch(
    ...
    if epoch_type != epoch_pb2.TRAIN and FLAGS.unroll_strategy in {
      "constant",
      "edge_count",
      "data_flow_max_steps",
      "label_convergence",
