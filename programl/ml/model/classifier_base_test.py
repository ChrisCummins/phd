...
class MockModel(classifier_base.ClassifierBase):
  ...
  def MakeBatch(
    ...
    return batches.BatchData(graph_ids=graph_ids, data=123)
...
def batch_iterator(
  ...
  return batches.BatchIterator(
    batches=model.BatchIterator(
      epoch.EpochType.TRAIN, graph_database_reader.BufferedGraphReader(graph_db)
    ),
...
def test_load_restore_model_from_checkpoint_using_tag(
  ...
  model(
    epoch_type=epoch.EpochType.VAL, batch_iterator=batch_iterator, logger=logger
  )
...
def test_empty_batches_is_not_error(
  ...
  model.Initialize()
  results = model(
    epoch_type=epoch.EpochType.TRAIN,
