...
class Batch(Base, sqlutil.PluralTablenameFromCamelCapsClassNameMixin):
  ...
  @property
  def epoch_type(self) -> epoch.EpochType:
    return epoch.EpochType(self.epoch_type_num)

  @epoch_type.setter
  def epoch_type(self, value: epoch.EpochType) -> None:
  ...
  def Create(
    ...
    results: programl.ml.batch.batch_results.BatchResults,
...
class Database(sqlutil.Database):
  ...
  def GetBestResults(
    ...
    run_id: run_id_lib.RunId,
    session: Optional[sqlutil.Database.SessionType] = None,
  ) -> Dict[epoch.EpochType, epoch.BestResults]:
    """Get the best results for a given run.

    Returns:
      A mapping from <epoch_type, epoch.EpochResults> for the best accuracy on each
      of the epoch types.
    """
    with self.Session(session=session) as session:
      ...
      for epoch_type in list(epoch.EpochType):
  ...
  def GetWeightedEpochStats(
    ...
    pdutil.RewriteColumn(df, "epoch_type", lambda x: epoch.EpochType(x))
    ...
    with self.Session(session=session) as session:
      ...
      for run_id in set(per_epoch_df["run_id"].values):
        ...
        for epoch_num in set(run_df["epoch_num"].values):
          ...
          for i, epoch_type in enumerate(list(epoch.EpochType)):
            epoch_df = run_df[
