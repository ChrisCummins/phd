...
import programl.ml.batch.rolling_results
...
class ClassifierBase(object):
  ...
  def __init__(
    ...
    self.epoch_num = 0
    self.best_results: Dict[epoch.EpochType, epoch.BestResults] = {
      epoch.EpochType.TRAIN: epoch.BestResults(),
      epoch.EpochType.VAL: epoch.BestResults(),
      epoch.EpochType.TEST: epoch.BestResults(),
    }
...
class EpochThread(progress.Progress):
  ...
  def __init__(
    ...
    self.results: epoch.EpochResults = None
    self.graph_ids = set()
  ...
  def Run(self) -> None:
    ...
    rolling_results = programl.ml.batch.rolling_results.RollingResults()
    ...
    self.results = epoch.EpochResults.FromRollingResults(rolling_results)
