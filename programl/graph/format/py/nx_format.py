...
def NetworkXToProgramGraph(
  ...
  """Convert a NetworkX graph to a ProgramGraph message.

  Args:
    g: A NetworkX MultiDiGraph.

  Returns:
    A ProgramGraph message.

  Raises:
    ValueError: If the graph cannot be converted.
  """
  proto = program_graph_pb2.ProgramGraph()
  node_link_graph = nx.readwrite.json_graph.node_link_data(g)
  serialized_proto = node_link_graph_pybind.NodeLinkGraphToProgramGraph(
    node_link_graph
  )
