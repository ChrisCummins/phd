...
PYBIND11_MODULE(node_link_graph_pybind, m) {
  ...
  m.def("NodeLinkGraphToProgramGraph",
        [&](const py::dict& obj) {
          // Convert python object to JSON.
          json dict = static_cast<py::object>(obj);

          ProgramGraph graph;
          Status status = NodeLinkGraphToProgramGraph(dict, &graph);
          if (!status.ok()) {
            throw std::runtime_error(status.ToString());
          }
        ...
        "Convert a JSON node link graph to program graph");
}
