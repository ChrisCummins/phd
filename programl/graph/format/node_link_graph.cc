...
// namespace detail {
//
// Status CreateNodeMessages(const json& nodes, ProgramGraph* graph) {
//  return Status::OK;
//}
//
// Status CreateEdgeMessages(const json& links, ProgramGraph* graph) {
//  return Status::OK;
//}
//
// Status CreateGraphMessages(const json& graphDict, ProgramGraph* graph) {
//  return Status::OK;
//}
//
//}  // namespace detail
//
Status NodeLinkGraphToProgramGraph(const json& dict, ProgramGraph* graph) {
  CHECK(graph) << "nullptr for output argument";
  //
  //  if (!dict["directed"]) {
  //    return Status(labm8::error::Code::INVALID_ARGUMENT,
  //                  "Input is not a directed graph");
  //  }
  //  if (!dict["multigraph"]) {
  //    return Status(labm8::error::Code::INVALID_ARGUMENT,
  //                  "Input is not a multigraph");
  //  }
  //  if (dict.find("graph") == dict.end()) {
  //    return Status(labm8::error::Code::INVALID_ARGUMENT,
  //                  "Missing key `graph` in node link graph");
  //  }
  //  if (dict.find("nodes") == dict.end()) {
  //    return Status(labm8::error::Code::INVALID_ARGUMENT,
  //                  "Missing key `nodes` in node link graph");
  //  }
  //  if (dict.find("links") == dict.end()) {
  //    return Status(labm8::error::Code::INVALID_ARGUMENT,
  //                  "Missing key `links` in node link graph");
  //  }
  //
  //  RETURN_IF_ERROR(detail::CreateGraphMessages(dict["graph"], graph));
  //  RETURN_IF_ERROR(detail::CreateNodeMessages(dict["nodes"], graph));
  //  RETURN_IF_ERROR(detail::CreateEdgeMessages(dict["links"], graph));
