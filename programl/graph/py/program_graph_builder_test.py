...
def test_linear_statement_control_flow():
  ...
  assert isinstance(builder.proto, program_graph_pb2.ProgramGraph)

  assert len(builder.proto.node) == 3

  assert builder.proto.node[builder.root].text == "<root>"
  assert builder.proto.node[builder.root].type == node_pb2.Node.INSTRUCTION

  assert builder.proto.node[a].text == "a"
  assert builder.proto.node[a].type == node_pb2.Node.INSTRUCTION
  assert builder.proto.node[a].function == fn

  assert builder.proto.node[b].text == "b"
  assert builder.proto.node[b].type == node_pb2.Node.INSTRUCTION
  assert builder.proto.node[b].function == fn

  assert len(builder.proto.edge) == 2
  assert builder.proto.edge[0].source == builder.root
  assert builder.proto.edge[0].target == a
  assert builder.proto.edge[0].position == 0
  assert builder.proto.edge[0].flow == edge_pb2.Edge.CONTROL

  assert builder.proto.edge[1].source == a
  assert builder.proto.edge[1].target == b
  assert builder.proto.edge[1].position == 0
  assert builder.proto.edge[1].flow == edge_pb2.Edge.CONTROL
