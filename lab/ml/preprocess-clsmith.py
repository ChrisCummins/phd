...
def is_modified(db):
    ...
    c.execute('SELECT MD5SUM(sha) FROM ContentFiles')
...
def preprocess_split(db_path, split):
    ...
    c.execute('SELECT sha,contents FROM ContentFiles LIMIT {} OFFSET {}'
    ...
    for row in rows:
        ...
        c.execute('SELECT sha FROM PreprocessedFiles WHERE sha=?', (sha,))
        result = c.fetchone()
