...
def test_grewe_features_df_columns(
  ...
  np.testing.assert_array_equal(
    df.columns.values,
    ["feature:grewe1", "feature:grewe2", "feature:grewe3", "feature:grewe4",],
  )
...
def test_df_gpu_runtimes_not_equal(
  ...
  assert not all(
    x == pytest.approx(y)
    for x, y in dataset.df[
      ["runtime:amd_tahiti_7970", "runtime:nvidia_gtx_960"]
    ].values
  )
