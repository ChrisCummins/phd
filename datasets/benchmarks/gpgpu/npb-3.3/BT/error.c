...
void rhs_norm(double rms[5]) {
  ...
  local_ws = temp == 0 ? 1 : temp;
  global_ws = clu_RoundWorkSize((size_t)(d2 - 2), local_ws);
  wg_num = global_ws / local_ws;
  ...
  ecode |= CECL_SET_KERNEL_ARG(k_rhs_norm, 3, sizeof(int), &d0);
  ecode |= CECL_SET_KERNEL_ARG(k_rhs_norm, 4, sizeof(int), &d1);
  ecode |= CECL_SET_KERNEL_ARG(k_rhs_norm, 5, sizeof(int), &d2);
  clu_CheckError(ecode, "CECL_SET_KERNEL_ARG()");
