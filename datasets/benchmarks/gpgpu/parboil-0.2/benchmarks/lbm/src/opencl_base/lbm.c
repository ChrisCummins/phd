...
void OpenCL_LBM_performStreamCollide(const OpenCL_Param* prm, cl_mem srcGrid,
  ...
  clStatus =
      CECL_SET_KERNEL_ARG(prm->clKernel, 0, sizeof(cl_mem), (void*)&srcGrid);
  CHECK_ERROR("CECL_SET_KERNEL_ARG")

  clStatus =
      CECL_SET_KERNEL_ARG(prm->clKernel, 1, sizeof(cl_mem), (void*)&dstGrid);
  CHECK_ERROR("CECL_SET_KERNEL_ARG")

  size_t dimBlock[3] = {SIZE_X, 1, 1};
  size_t dimGrid[3] = {SIZE_X * SIZE_Y, SIZE_Z, 1};
  clStatus = CECL_ND_RANGE_KERNEL(prm->clCommandQueue, prm->clKernel, 3, NULL,
...
void OpenCL_LBM_initializeGrid(const OpenCL_Param* prm, cl_mem d_grid,
  ...
  const size_t size = TOTAL_PADDED_CELLS * N_CELL_ENTRIES * sizeof(float);
  cl_int clStatus;
  clStatus = CECL_WRITE_BUFFER(prm->clCommandQueue, d_grid, CL_TRUE, 0, size,
...
void OpenCL_LBM_getDeviceGrid(const OpenCL_Param* prm, cl_mem d_grid,
  ...
  const size_t size = TOTAL_PADDED_CELLS * N_CELL_ENTRIES * sizeof(float);
  cl_int clStatus;
  clStatus = CECL_READ_BUFFER(prm->clCommandQueue, d_grid, CL_TRUE, 0, size,
                              h_grid - MARGIN, 0, NULL, NULL);
