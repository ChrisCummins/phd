...
class BenchImpl(object):
  ...
  def isBuilt(self, benchmark, platform):
    ...
    return self.makefile(
        benchmark, action='q', platform=platform) == ErrorType.Success
  ...
  def run(self,
    ...
    def perform():
      ...
      try:
        rc = self.makefile(
            benchmark, action='run', platform=platform, opt={"ARGS": args})
  ...
  def debug(self,
    ...
    def perform():
      ...
      ###
      rc = self.makefile(
          benchmark, action='debug', platform=platform, opt={"ARGS": args})
  ...
  def check(self, benchmark, dataset):
    ...
    def perform():
      ...
      rc = process.spawnwaitl(compare, compare, reference_file, output_file)
...
class BenchDataset(object):
  ...
  def getTemporaryOutputFile(self, benchmark):
    ...
    return self.getTemporaryOutputDir(benchmark).getChildByName(
        self.outFiles[0])
  ...
  def getCommandLineArguments(self, benchmark, do_output=True):
    ...
    if self.inFiles:
      in_files = ",".join(
          [path.join(self.dir.getPath(), 'input', x) for x in self.inFiles])
      args.append("-i")
...
def find_benchmarks():
  ...
  try:
    ...
    for bmkdir in globals.benchdir.getScannedChildren():
      bmk = Future(lambda bmkdir=bmkdir: Benchmark.createFromName(bmkdir.
                                                                  getName()))
...
def benchmark_scanner():
  ...
  return lambda x: pbf.scan_file(x, True, create_benchmark_dir,
                                 ['_darcs', '.svn'])
...
def dataset_scanner():
  ...
  def create_dataset_dir(dpath):
    ...
    expected = [
        pbf.Directory(
            path.join(dpath, 'input'), [_desc_file(path.join(dpath, 'input'))],
            simple_scan),
  ...
  return lambda x: pbf.scan_file(x, True, create_dataset_dir,
                                 ['.svn', '_darcs'])
