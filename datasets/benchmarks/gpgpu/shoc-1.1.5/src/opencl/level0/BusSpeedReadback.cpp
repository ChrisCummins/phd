                  ...
                  OptionParser &op)
{
    ...
    {
	int err1, err2;
        hostMemObj1 = CECL_BUFFER(ctx,
                                     CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
                                     sizeof(float)*numMaxFloats, NULL, &err1);
        if (err1 == CL_SUCCESS)
        {
            hostMem1 = (float*)CECL_MAP_BUFFER(queue, hostMemObj1, true,
                                                  CL_MAP_READ|CL_MAP_WRITE,
                                                  0,sizeof(float)*numMaxFloats,0,
                                                  NULL,NULL,&err1);
        }
        hostMemObj2 = CECL_BUFFER(ctx,
                                     CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
                                     sizeof(float)*numMaxFloats, NULL, &err2);
        if (err2 == CL_SUCCESS)
        {
            hostMem2 = (float*)CECL_MAP_BUFFER(queue, hostMemObj2, true,
                                                  CL_MAP_READ|CL_MAP_WRITE,
                                                  0,sizeof(float)*numMaxFloats,0,
                                                  NULL,NULL,&err2);
        }
    ...
    for (int pass = 0; pass < npasses; pass++)
    {
        ...
        for (int i = 0; i < nSizes; i++)
        {
            ...
            err = CECL_READ_BUFFER(queue, mem1, false, 0,
