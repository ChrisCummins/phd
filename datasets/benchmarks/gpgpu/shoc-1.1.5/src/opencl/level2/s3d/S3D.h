...
#define clProg(name, source)                                                   \
	cl_program name = CECL_PROGRAM_WITH_SOURCE(ctx, 1,                    \
            &source, NULL, &err);                                              \
    CL_CHECK_ERROR(err)


#define clMalloc(ptr, size) { ptr =                                            \
    CECL_BUFFER(ctx, CL_MEM_READ_WRITE, size, NULL, &err);                  \
    CL_CHECK_ERROR(err); }

#define clMemtoDevice(ptr1, ptr2, size) {                                      \
    err = CECL_WRITE_BUFFER(queue, ptr1, true, 0, size, ptr2, 0,            \
...
#define clBuild(progname)                                                      \
    err = CECL_PROGRAM(progname, 0, NULL,                                    \
...
#define clSetRatxArg(kernel_name) {                                            \
  ...
  CL_CHECK_ERROR(err);                                                         \
  err = CECL_SET_KERNEL_ARG(kernel_name, 4, sizeof(cl_mem), (void*)&d_rklow);       \
  CL_CHECK_ERROR(err);                                                         \
...
#define clSetRdwdotArg(kernel_name) {                                          \
  ...
  CL_CHECK_ERROR(err);                                                         \
  err = CECL_SET_KERNEL_ARG(kernel_name, 4, sizeof(cl_mem), (void*)&d_molwt);       \
  CL_CHECK_ERROR(err);                                                         \
  }

#define clLaunchKernel(kernel_name) {                                          \
    err = CECL_ND_RANGE_KERNEL(queue, kernel_name,                           \
