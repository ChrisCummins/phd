...
void IMGVF_OpenCL(MAT **I, MAT **IMGVF, double vx, double vy, double e,
  ...
  if (!compiled) {
    ...
    // Compile the kernel
    cl_program program =
        CECL_PROGRAM_WITH_SOURCE(context, 1, &source, &sourceSize, &error);
  ...
  CECL_SET_KERNEL_ARG(IMGVF_kernel, 0, sizeof(cl_mem),
                      (void *)&device_IMGVF_all);
  CECL_SET_KERNEL_ARG(IMGVF_kernel, 1, sizeof(cl_mem), (void *)&device_I_all);
  CECL_SET_KERNEL_ARG(IMGVF_kernel, 2, sizeof(cl_mem),
                      (void *)&device_I_offsets);
  ...
  error =
      CECL_ND_RANGE_KERNEL(command_queue, IMGVF_kernel, 1, NULL,
...
void IMGVF_OpenCL_init(MAT **IE, int num_cells) {
  ...
  device_I_offsets =
      CECL_BUFFER(context, CL_MEM_READ_ONLY, mem_size, NULL, &error);
  check_error(error, __FILE__, __LINE__);
  ...
  device_I_all =
      CECL_BUFFER(context, CL_MEM_READ_ONLY, total_mem_size, NULL, &error);
  check_error(error, __FILE__, __LINE__);
  device_IMGVF_all =
      CECL_BUFFER(context, CL_MEM_READ_WRITE, total_mem_size, NULL, &error);
  check_error(error, __FILE__, __LINE__);
  ...
  for (cell_num = 0; cell_num < num_cells; cell_num++) {
    ...
    // Copy matrix I (which is also the initial IMGVF matrix) into the overall
    // array
    for (i = 0; i < m; i++)
      for (j = 0; j < n; j++)
  ...
  error = CECL_WRITE_BUFFER(command_queue, device_I_all, CL_TRUE, 0,
                            total_mem_size, (void *)host_I_all, 0, NULL, NULL);
  check_error(error, __FILE__, __LINE__);
  error = CECL_WRITE_BUFFER(command_queue, device_IMGVF_all, CL_TRUE, 0,
                            total_mem_size, (void *)host_I_all, 0, NULL, NULL);
  check_error(error, __FILE__, __LINE__);

  // Copy offsets array to device
  error = CECL_WRITE_BUFFER(command_queue, device_I_offsets, CL_TRUE, 0,
                            mem_size, (void *)host_I_offsets, 0, NULL, NULL);
  check_error(error, __FILE__, __LINE__);

  // Copy memory dimension arrays to device
  error = CECL_WRITE_BUFFER(command_queue, device_m_array, CL_TRUE, 0, mem_size,
                            (void *)host_m_array, 0, NULL, NULL);
  check_error(error, __FILE__, __LINE__);
  error = CECL_WRITE_BUFFER(command_queue, device_n_array, CL_TRUE, 0, mem_size,
                            (void *)host_n_array, 0, NULL, NULL);
  check_error(error, __FILE__, __LINE__);
}
...
void IMGVF_OpenCL_cleanup(MAT **IMGVF_out_array, int num_cells) {
  ...
  float *host_IMGVF_all = (cl_float *)CECL_MAP_BUFFER(
