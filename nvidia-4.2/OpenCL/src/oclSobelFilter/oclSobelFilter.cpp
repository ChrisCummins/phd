...
int main(int argc, char** argv)
{
    ...
    for (cl_uint i = 0; i < GpuDevMngr->uiUsefulDevCt; i++)
    {
        ...
        ckSobel[i] = CECL_KERNEL(cpProgram, "ckSobel", &ciErrNum);
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
        shrLog("CECL_KERNEL (ckSobel), Device %u...\n", i); 
        ...
        ciErrNum = CECL_SET_KERNEL_ARG(ckSobel[i], 0, sizeof(cl_mem), (void*)&cmDevBufIn[i]);
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 1, sizeof(cl_mem), (void*)&cmDevBufOut[i]);
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 2, (iLocalPixPitch * (iBlockDimY + 2) * sizeof(cl_uchar4)), NULL);
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 3, sizeof(cl_int), (void*)&iLocalPixPitch);
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 4, sizeof(cl_uint), (void*)&uiImageWidth);
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 6, sizeof(cl_float), (void*)&fThresh);        
        oclCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
...
double SobelFilterGPU(cl_uint* uiInputImage, cl_uint* uiOutputImage)
{
    ...
    for (cl_uint i = 0; i < GpuDevMngr->uiUsefulDevCt; i++)
    {
        ...
        ciErrNum |= CECL_SET_KERNEL_ARG(ckSobel[i], 5, sizeof(cl_uint), (void*)&uiDevImageHeight[i]);

        // Launch Sobel kernel(s) into queue(s) and push to device(s)
        ciErrNum |= CECL_ND_RANGE_KERNEL(cqCommandQueue[i], ckSobel[i], 2, NULL, szGlobalWorkSize, szLocalWorkSize, 0, NULL, NULL);
