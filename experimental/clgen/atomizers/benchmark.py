...
class FastAtomizer(Atomizer):
    ...
    try:
      ...
      while j < len(text):
        ...
        if k == 1:
          ...
          i = j - 1
          j -= 1
        elif not any(len(x) >= k and x[0] == text[i] and
                     x[k - 1] == text[j - 1]
                     for x in atoms):
          indices.append(self.vocab[text[i:j - 1]])
          i = j - 1
  ...
  def from_text(text: str) -> Atomizer:
    ...
    while j < len(text):
      j += 1
      k = j - i
      if k >= 1 and not any(len(x) >= k and
                            x[0] == text[i] and
                            x[k - 1] == text[j - 1]
...
def main():
  ...
  print("CharacterAtomizer", timeit("chara.atomize(data)",
                                    "from __main__ import data, chara"))
  # print("GreedyAtomizer   ", timeit("greedy.atomize(data)",
  #                                   "from __main__ import data, greedy"))
  print("FastAtomizer     ", timeit("fast.atomize(data)",
                                    "from __main__ import data, fast"))
