...
def get_num_to_run(
  ...
  num_ran = session.query(sql.sql.func.count(CLgenResult.id)).filter(
  ...
  if optimizations is not None:
    num_ran = (
      num_ran.join(CLgenTestCase)
      .join(cldriveParams)
      .filter(cldriveParams.optimizations == optimizations)
    )
    total = total.join(cldriveParams).filter(
      cldriveParams.optimizations == optimizations
    )
  ...
  with Session(commit=False) as session:
    ...
    def next_batch():
      ...
      if optimizations is not None:
          ...
          done.join(CLgenTestCase)
          .join(cldriveParams)
          .filter(cldriveParams.optimizations == optimizations)
        )

      todo = (
        session.query(CLgenTestCase)
        .filter(~CLgenTestCase.id.in_(done))
        .order_by(CLgenTestCase.id)
      )
      if optimizations is not None:
        todo = todo.join(cldriveParams).filter(
          cldriveParams.optimizations == optimizations
        )
      ...
      while True:
        ...
        try:
          ...
          stdout = get_or_create(
            session, CLgenStdout, hash=crypto.sha1_str(stdout_), stdout=stdout_
          )

          stderr_ = util.escape_stderr(stderr)
          stderr = get_or_create(
            session, CLgenStderr, hash=crypto.sha1_str(stderr_), stderr=stderr_
          )
          session.flush()
