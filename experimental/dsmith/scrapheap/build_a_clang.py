...
def get_num_programs_to_build(
  session: db.session_t, tables: Tableset, clang: str
):
  num_ran = (
    session.query(sql.sql.func.count(tables.clangs.id))
    .filter(tables.clangs.clang == clang)
    .scalar()
  )
  total = session.query(sql.sql.func.count(tables.programs.id)).scalar()
  return num_ran, total


def create_stderr(
  s: session_t, tables: Tableset, stderr: str
) -> CLgenClangStderr:
...
def build_with_clang(
  program: Union[CLgenProgram, CLSmithProgram], clang: str
) -> Tuple[int, float, str]:
...
if __name__ == "__main__":
  ...
  with Session(commit=True) as s:
    ...
    def next_batch():
      ...
      # fill inbox
      done = s.query(tables.clangs.program_id).filter(
        tables.clangs.clang == args.clang
      )
      todo = (
        s.query(tables.programs)
    ...
    for tables in tablesets:
      if args.recheck:
        ...
        for stderr in ProgressBar(max_value=q.count())(q):
          assertion_ = util.get_assertion(
            s, tables.clang_assertions, stderr.stderr
          )
          unreachable_ = util.get_unreachable(
            s, tables.clang_unreachables, stderr.stderr
          )
          terminate_ = util.get_terminate(
            s, tables.clang_terminates, stderr.stderr
          )
        ...
        while True:
          ...
          q = (
            s.query(tables.clang_stderrs.id)
