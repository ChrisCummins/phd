...
brew_cask(
    name = "omnifocus_app",
    outs = ["/Applications/OmniFocus.app"],
    platforms = ["osx"],
)

task_group(
    name = "omnifocus",
    deps = ["omnifocus_app", "omni", "of2"],
)
...
def NeedToInstallOf2():
    return (os.path.exists('/usr/local/opt/ofexport/bin/of2') and
...
task_group(
    name = "bazel",
    deps = ["bazel_brew_app", "bazel_apt_app"]
)
