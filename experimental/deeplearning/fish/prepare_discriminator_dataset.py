...
app.DEFINE_string(
  "export_path",
  "~/data/experimental/deeplearning/fish/75k",
...
app.DEFINE_list(
  "positive_class_outcomes",
  ["build_crash"],
  "The outcomes to select positive examples from.",
)
app.DEFINE_list(
  "negative_class_outcomes",
  ["pass"],
...
app.DEFINE_boolean(
  "include_bf_outcomes_as_negative",
  False,
...
def GetProtos(
  export_path: pathlib.Path, outcomes: typing.List[str], max_src_len: int
) -> typing.List[TrainingProto]:
  paths = sorted(
    labtypes.flatten(
...
def LoadPositiveProtos(
  ...
  protos = [
    p
    for p in GetProtos(export_path, positive_class_outcomes, max_src_len)
...
def LoadNegativeProtos(
  export_path: pathlib.Path,
  positive_protos: typing.List[TrainingProto],
  negative_class_outcomes: typing.List[str],
  max_src_len: int,
  ...
  if balance_class_lengths:
    ...
    app.Log(
      1,
      "Loaded %s negative protos. Balancing lengths ...",
    ...
    for i, positive_proto_size in enumerate(positive_proto_sizes):
      ...
      app.Log(
        ...
        humanize.Commas(positive_proto_size),
        size_diffs.min(),
      )
  ...
  app.Log(
    1, "Loaded %s negative data protos", humanize.Commas(len(negative_protos))
  )
...
def main(argv):
  ...
  positive_protos, negative_protos = LoadNegativeProtos(
    export_path,
    positive_protos,
    FLAGS.negative_class_outcomes,
    FLAGS.max_src_len,
    FLAGS.balance_class_lengths,
