...
def main(argv):
  ...
  ratios = DatasetRatios(
      FLAGS.training_ratio, FLAGS.validation_ratio, FLAGS.test_ratio)
  assert sum(ratios) <= 1
  ...
  testing_size = int(len(positive_protos) * FLAGS.testing_size)
  ...
  for i, proto in enumerate(positive_protos[:training_size]):
    pbutil.ToFile(fish_pb2.DiscriminatorDatapoint(
        input=proto.src, label=fish_pb2.DiscriminatorDatapoint.POSITIVE),
