...
""",
  ...
  for row in cursor:
    ...
    if testbed_name == "clang":
      ...
      _SetIf(
        testbed_opts, "opencl_devtype", OPENCL_DEVTYPE_MAP.get(devtype, devtype)
      )
    ...
    testcase = deepsmith_pb2.Testcase(
      ...
      profiling_events=[
        deepsmith_pb2.ProfilingEvent(
          ...
          event_start_epoch_ms=labdate.MillisecondsTimestamp(program_date),
        ),
      ],
    )
...
def _ExportProtos() -> None:
  ...
  app.Log(
    1,
    "Exported %d testcases and %d results",
