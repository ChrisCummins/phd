...
app.DEFINE_boolean(
  "multithreaded_preprocess", True, "Use multiple threads during preprocessing."
)
app.DEFINE_integer(
  "preprocess_worker_threads",
...
app.DEFINE_integer(
  "preprocessor_input_query_size",
  1024,
  "The number of repo URLs to fetch per query.",
)
app.DEFINE_integer(
  "preprocessor_worker_repo_count",
  16,
...
def BatchedReposIterator(input_db, batch_size):
  with input_db.Session() as input_session:
    if FLAGS.reverse_order:
      last_date = input_session.query(
        sql.func.max(contentfiles.GitHubRepository.date_scraped)
      ).one()
    else:
      last_date = input_session.query(
        sql.func.min(contentfiles.GitHubRepository.date_scraped)
      ).one()
  ...
  while True:
    with input_db.Session() as input_session:
      ...
      if FLAGS.reverse_order:
        query = query.filter(
          contentfiles.GitHubRepository.date_scraped <= last_date
...
def PreprocessStringList(
...
) -> internal_pb2.PreprocessorWorkerJobOutcomes:
  ...
  try:
    ...
    if len(srcs) > 1:
      # Divide and conquer.
      app.Log(
        1, "JavaPreprocessor failed processing %d srcs, dividing", len(srcs)
      )
      mid = int(len(srcs) / 2)
      ...
      app.Error(
        "JavaPreprocessor failed processing message written to %s", path
      )
...
def PreprocessContentfiles(
  ...
  assert (
    len(cfs)
    == len(output_message.outcome)
    == len(output_message.preprocess_time_ms)
  )
...
class Preprocessor(threading.Thread):
  ...
  def ProcessABatchOfRepos(self, clone_from_urls: typing.List[str]) -> bool:
    ...
    with self.output_db.Session(commit=True) as output_session:
      with self.input_db.Session(commit=True) as input_session:
        input_session.query(contentfiles.GitHubRepository).filter(
    ...
    app.Log(
      1,
      "Preprocessed %s Java methods at a rate of %d ms per method",
  ...
  def run(self):
    ...
    repos = Chunkify(
      BatchedReposIterator(self.input_db, FLAGS.preprocessor_input_query_size),
