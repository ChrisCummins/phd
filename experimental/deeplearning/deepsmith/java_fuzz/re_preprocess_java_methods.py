...
""".encode(
    ...
    if outcome.status != internal_pb2.PreprocessorWorkerJobOutcome.OK:
      ...
      hashlib.sha256(outcome.contents.encode("utf-8")).hexdigest()
      != pp_cf.sha256
    ):
...
class RePreprocessor(threading.Thread):
  ...
  def run(self):
    ...
    app.Log(
      ...
      humanize.Commas(len(ids_to_preprocess)),
      max_workers,
    )
...
def GetPreprocessedCount(outdir: pathlib.Path) -> int:
  return (
    len(list((outdir / "pass").iterdir()))
    + len(list((outdir / "fail").iterdir()))
...
def Repreprocess(input_db, pp_db, outdir: pathlib.Path):
  ...
  with pp_db.Session() as s:
    cf_count = (
      s.query(preprocessed.PreprocessedContentFile)
      .filter(
        preprocessed.PreprocessedContentFile.preprocessing_succeeded == True
      )
      .count()
