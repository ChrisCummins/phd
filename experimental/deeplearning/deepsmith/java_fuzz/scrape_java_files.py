...
FLAGS = app.FLAGS
app.DEFINE_integer(
  "n", int(1e6), "The number of repos to scrape before terminating."
)
app.DEFINE_database(
  "db",
...
class FuzzyGitHubJavaScraper(object):
  ...
  def DoRun(self, tempdir: str, n: int):
    ...
    for word in self.word_list:
      ...
      scraper = pipelined_scraper.PipelinedScraper(
        language, language.query[0], self.connection, self.db
      )
      app.Log(
        1,
      ...
      while scraper.is_alive():
        ...
        if i + query_i >= n:
          app.Log(
            1, "Scraped %s repositories. Done.", humanize.Commas(i + query_i)
          )
          scraper.Stop()
          scraper.join()
          break
...
def main():
  ...
  app.Log(
    1,
    "Beginning fuzzy GitHub scraping using %s words ...",
