...
app.DEFINE_boolean(
  "ls_env", False, "List the available OpenCL devices and exit."
)
app.DEFINE_string(
  "generator", "clgen", "The type of generator to use. One of: {clgen,clsmith}."
)
app.DEFINE_string(
  "generator_config",
  None,
...
app.DEFINE_boolean(
  "opencl_opt", True, "If --noopencl_opt, OpenCL optimizations are disabled."
)
app.DEFINE_string(
...
def TestingLoop(
  ...
  batch_num = 0
  while (
    num_interesting_results < min_interesting_results
...
def GeneratorFromFlag(
  config_class, generator_class
) -> base_generator.GeneratorServiceBase:
...
def GetGenerator() -> base_generator.GeneratorServiceBase:
  ...
  if FLAGS.generator == "clgen":
    generator = GeneratorFromFlag(
      generator_pb2.ClgenGenerator, clgen_pretrained.ClgenGenerator
    )
  elif FLAGS.generator == "clsmith":
    generator = GeneratorFromFlag(
      generator_pb2.ClsmithGenerator, clsmith.ClsmithGenerator
    )
  else:
    raise app.UsageError(
      f"Unrecognized value for --generator: '{FLAGS.generator}'"
...
def ReRunResult(
  result: deepsmith_pb2.Result, dut_harness: base_harness.HarnessBase
  ...
  app.Warning(
    f"Re-run result has same outcome: "
    f"{new_result.outcome == result.outcome}."
  )
  app.Warning(
    f"Re-run result has same returncode: "
