...
app.DEFINE_integer(
  "output_corpus_size",
  10000,
  "The minimum number of samples to generate in the output" "corpus.",
)
...
def SampleModel(instance: clgen.Instance) -> None:
  ...
  app.Log(
    1,
    "Need to generate %d samples in %s",
    max(target_samples - num_samples, 0),
    sample_dir,
  )
...
def main(argv):
  ...
  instances = [
    clgen.Instance(p)
    for p in pbutil.FromFile(
      pathlib.Path(FLAGS.instances), clgen_pb2.Instances()
    ).instance
  ]
  ...
  app.Log(
    1,
    "Loaded %d instances in %s ms",
    len(candidate_instances),
