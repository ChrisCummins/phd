...
app.DEFINE_string(
  "opencl_env",
  oclgrind.CLINFO_DESCRIPTION.name,
...
def OpenClSourceToTestCases(src: str) -> typing.List[deepsmith_pb2.Testcase]:
  ...
  return [
    deepsmith_pb2.Testcase(
      ...
      inputs={"src": src, "gsize": f"{gsize},1,1", "lsize": f"{lsize},1,1",},
    )
...
def RunTestCasesOrDie(
  driver: cldrive.CldriveHarness, testcases: typing.List[deepsmith_pb2.Testcase]
  ...
  response = driver.RunTestcases(
    harness_pb2.RunTestcasesRequest(
      testbed=driver.testbeds[0], testcases=testcases,
    ),
    None,
  )
...
def GetOutcomeWithDynamicChecks(
  result: deepsmith_pb2.Result, driver: cldrive.CldriveHarness
) -> str:
  ...
  if (
    len(set(r.outputs["stdout"] for r in [result, different_input_results[0]]))
    == 1
  ):
...
def main(argv: typing.List[str]):
  ...
  driver = cldrive.CldriveHarness(
    harness_pb2.CldriveHarness(
      opencl_env=[FLAGS.opencl_env], opencl_opt=[FLAGS.opencl_opt],
    )
  )
