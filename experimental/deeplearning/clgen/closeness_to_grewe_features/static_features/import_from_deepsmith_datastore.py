...
app.DEFINE_string(
  "datastore", None, "Path of the datastore config to import form"
)
app.DEFINE_integer(
  "batch_size", 256, "The number of testcases to process in a batch."
)
...
def main(argv: typing.List[str]):
  ...
  with ds.Session(commit=False) as session:
    generator_id = (
      session.query(generator.Generator.id)
      .filter(generator.Generator.name == "clgen")
      .first()
    )
    if not generator_id:
    ...
    toolchain_id = (
      session.query(toolchain.Toolchain.id)
      .filter(toolchain.Toolchain.string == "opencl")
      .first()
    )
    if not toolchain_id:
    ...
    q = (
      session.query(testcase.Testcase)
