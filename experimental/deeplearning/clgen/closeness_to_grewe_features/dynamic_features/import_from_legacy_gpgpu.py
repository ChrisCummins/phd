...
def DynamicFeaturesFromKernelInvocation(
  kernel_invocation: libcecl_pb2.OpenClKernelInvocation, static_features_id: int
) -> db.DynamicFeatures:
  return {
...
def GetStaticFeaturesId(session, origin, kernel_name, log_lines):
  ids = (
    session.query(db.StaticFeatures.id)
