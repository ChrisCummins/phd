...
app.DEFINE_string('device_name', None, 'The name of the OpenCL device',
                  required=True)
app.DEFINE_string('device_type', None, 'The device type. One of: {CPU, GPU}',
                  required=True, validator=lambda val: val in {"CPU", "GPU"})
app.DEFINE_string('opencl_env', None, 'The name of the OpenCL environment.',
                  required=True)
app.DEFINE_string('hostname', None, 'The name of the hostname', required=True)


def DynamicFeaturesFromKernelInvocation(
    kernel_invocation: libcecl_pb2.OpenClKernelInvocation,
    static_features_id: int, env_name: str, hostname: str) -> db.DynamicFeatures:
...
def ImportFromLegacyGpgpu(session, logs_zip: pathlib.Path,
                          expected_devtype: str, expected_device_name: str,
                          env_name: str, hostname: str) -> None:
      ...
      for device_dir in run_dir.iterdir():
        for benchmark_log in device_dir.iterdir():
          ...
          kernel_invocations = libcecl_runtime.KernelInvocationsFromCeclLog(
              ...
              expected_devtype=expected_devtype, expected_device_name=expected_device_name)
          ...
          for kernel_invocation in kernel_invocations:
            ...
            session.add(DynamicFeaturesFromKernelInvocation(
                kernel_invocation, static_features_id)

def main():
  """Main entry point."""
  with FLAGS.db.Session(commit=True) as session:
    ImportFromLegacyGpgpu(
        session, FLAGS.gpgpu_logs_zip, expected_devtype=FLAGS.device_type,
        expected_device_name=FLAGS.device_name,
        env_name=FLAGS.opencl_env, hostname=FLAGS.hostname)
