...
def GetBenchmarkSuiteToRun(
  session: sqlutil.Session, env: cldrive_env.OpenCLEnvironment
) -> typing.Optional[str]:
  """Get the name of a benchmark suite to run."""
  already_done = session.query(db.DynamicFeatures.static_features_id).filter(
    db.DynamicFeatures.opencl_env == env.name,
    db.DynamicFeatures.driver == db.DynamicFeaturesDriver.LIBCECL,
  )
  q = (
    session.query(db.StaticFeatures.origin)
...
class DatabaseObserver(gpgpu.BenchmarkRunObserver):
  ...
  def OnBenchmarkRun(self, log: gpgpu_pb2.GpgpuBenchmarkRun):
    for kernel_invocation in log.run.kernel_invocation:
      ...
      origin = (
        f"benchmarks_{log.benchmark_suite}:{log.benchmark_name}:"
        f"{log.dataset_name}"
      )
    ...
    app.Log(
      1, "Processed %d kernel invocations", len(log.run.kernel_invocation)
    )
...
def DriveBenchmarkSuiteAndRecordResults(
  ...
  with benchmark_suite_class() as benchmark_suite:
    ...
    for i in range(num_runs):
      app.Log(
        1, "Starting run %d / %d of %s", i + 1, num_runs, benchmark_suite.name
      )
...
def main():
  ...
  while True:
    ...
    if not benchmark_suite:
      ...
      return
    try:
      DriveBenchmarkSuiteAndRecordResults(
        database, benchmark_suite, env, FLAGS.num_runs
      )
