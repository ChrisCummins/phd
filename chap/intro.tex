...
Machine learning systems are fundamentally probabilistic. For conditional situations in which there is a categorically incorrect choice, a machine learning model will have a non-zero probability of making that choice. While some of the work within compilers can 

The final challenge which must be overcome is that machine learning practices must be widely adopted by the compiler community. For new techniques to be adopted they must provide clear advantages over existing approaches. XXX momentum

Compiler construction typically uses formal systems to , whereas machine learning systems are fundamentally probabilistic. Techniques must be developed for these probabilistic systems to coexist with formal approaches, without sacrificing correctness. Compilers thousands of choices for which correctness is not a consideration, such as optimisation decisions. There are many cases for which probabilistic systems may be used used, such as in optimisation decisions.

If machine learning simply changes the

In other cases, such as compiler testing, heuristic methods may be combined with probabilistic methods to prevent errors.??
...
\diff{%
The promise of machine learning techniques is reduced compiler development cost and improved performance. Achieving these goal requires overcoming three significant challenges: the data scarcity challenge, the feature design challenge, and the adoption challenge. Subsequent chapters describe novel techniques which address these challenges.%
}
