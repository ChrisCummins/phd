...
\diff{%
Compilers, which play an essential role in extracting software  efficiency, have changed little in the last few decades.
% They comprise a front-end, back-end, and optimising "middle-end". While some of the logic can be shared across architectures, the optimisation decisions to extract the best performance from specific hardware cannot.
Compilers comprise millions of lines of code and are too complex to be fully understood by a single developer, yet optimising a single component requires knowldge of the full compiler and the interaction of its components. Currently, each architecture requires extensive hand tuning by experts to extract good performance.%
}
