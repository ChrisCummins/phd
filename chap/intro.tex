...
\diff{%
There has been an unprecedented increase in the scale and quantity of data-intensive workloads. The energy consumption of the ICT is estimated to rise to 20\% of global output by 2030~\cite{Andrae2019}. Fundamental shifts in both hardware and software are required to meet the demands of the transition to \emph{big data}~\cite{Hashem2015,Kanev2016,Gandomi2015}.%
}

\diff{%
Compilers, which play an essential role in extracting performance, have changed little in the last few decades. They comprise a front-end, back-end, and optimising "middle-end". While some of the logic can be shared across architectures, the optimisation decisions to extract the best performance from specific hardware cannot. Currently, each architecture requires extensive hand tuning by experts to extract good performance. Optimising a single component requires knowledge of the full compiler and the interaction of components, yet compilers comprise millions of lines of code and are too complex to be fully understood by a single developer. This makes them cumbersome and unable to adapt to the rapidly changing hardware landscape arising from the of GPGPU, FPGA~\cite{Misra2010}, ASICs~\cite{Misra2010,Jouppi2017}.
%
}
