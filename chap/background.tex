...
This chapter provides a brief and non-exhaustive overview of the techniques and theory used in this thesis. First Section~\ref{sec:background-terminology} defines terminology. Then Section~\ref{sec:background-machine-learning} describes the machine learning techniques used in this thesis. Section~\ref{sec:evaluation-techniques} describes the statistical tools and methodologies used in this thesis. Section~\ref{sec:programming-heterogeneous-systems} provides an overview of heterogeneous programming. Lastly Section~\ref{sec:background-summary} concludes.


\section{Terminology}
\label{sec:background-terminology}

\emph{Machine learning} refers to a family of statistical models and algorithms used to infer functions to estimate future values given past example values and their \emph{features}.

\emph{Features}, or explanatory variables, are a set of observable attributes used by machine learning algorithms to build the correlations required to predict unseen values. % A \emph{feature space} is the

\emph{Deep learning} is a loosely defined class of machine learning methods built on artificial neural networks. It is easily confused with \emph{deep neural networks}, which is a specific deep learning technique employing artificial neural networks with one or more hidden layers. Recurrent Neural Networks, as used in this thesis, are a deep learning architecture.


\section{Machine Learning}
\label{sec:background-machine-learning}

Machine learning techniques are used in this thesis for classification and sequence-to-sequence modelling. Classification is the task of predicting the correct category --- or class --- for a set of features, based on ``labelled'' training data, i.e.\ instances whose categories are known. Sequence modelling is the task of capturing the underling probability distribution describing a sequence of values. This section briefly describes the classification and sequential modelling techniques used in this thesis.
...
The \emph{Leaky ReLU} variation addresses the issue of large adjustments to parameters during training leading to neurons that are not activated for any input, effectively ``dying''. Leaky ReLU prevents artificial neurons becoming unresponsive to variations in input by introducing a small slope for negative values:
...
Ordinary backpropagation may be used on an RNN by unfolding the computation graph over time, shown in Figure~\subref{fig:rnn-unrolled}. This \emph{Backpropagation Through Time} (BPTT)~\cite{Werbos1990a} enables the propagation of errors through time in the same manner as through layers.
...
Where weight matrices $\bm{W} \in \mathbb{R}^{h\times d}$ and $\bm{U} \in \mathbb{R}^{h\times h}$, and bias vectors $\bm{b} \in \mathbb{R}^h$ are sub-scripted for the activation being calculated: $i$ input gate, $o$ output gate, $f$ forget gate, or $c$ cell state. The cell state and hidden state are then updated using:
...
Decision trees are popular and low overhead form of classification. Implementations can be as simple and efficient as a set of nested \texttt{if}/\texttt{else} statements.


\subsection{ZeroR}

A ``ZeroR'' classifier is a model whose output is always the mode of the training data classes, irrespective of the input features. For example, given training data with the labels $\bm{y} = { A, B, B, C, }$, the model will predict $\bm{\hat{y}} = B$ for all future values. This requires no feature values $x$ since a ZeroR classifier has no power of prediction. A ZeroR model represents the simplest approach to classification, and is useful for providing a baseline to evaluate the performance of more complex classifiers.
...
When $K=N$, this is known as \emph{leave-one-out} cross-validation. For a classification task, if observations are distributed amongst the $K$ partitions such that the distribution of observation classes in all partitions is equal, this is called \emph{stratified} $K$-fold cross-validation.
...
To compute the principle components, one begins with a normalized matrix of observations $\bm{X} \in \mathbb{R}^{m \times n}$ of $m$ rows and $n$ columns (components). The data is normalised to the range $[-1,1]$ by subtracting the mean and scaling each component by its variance. The matrix of $k$ principle components $\bm{\hat{X}} \in \mathbb{R}^{m \times k}$ can then be computed from eigenvectors $\bm{v}^{(1)}, \ldots \bm{v}^{(m)}$ using:
...
This chapter has detailed the machine learning techniques used in this thesis, the techniques used in evaluation, and the domain in which they are evaluated, heterogeneous programming with OpenCL. The following chapter surveys the research literature relevant to this work.
