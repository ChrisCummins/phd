...
%  }\\%
  ...
  \subfloat[Testbeds $1\pm$, $2\pm$ segmentation fault due to invalid pointer assignment.]{%
...
\begin{figure}
  ...
  \caption{Example kernels which induce failures in specific compiler passes. \cc{Pick out more examples, and a more diverse range}}%
\end{figure}


\begin{figure}
  \centering %
  \subfloat[Testbeds $1\pm$, $2\pm$, $3\pm$ reject automatic variables with address-space qualifiers (line 4).]{%
    \noindent\mbox{\parbox{\columnwidth}{\usebox{\AddressQualifiedAutoVar}}}%
    \label{lst:adress-qualified-autovar}
  }\\%
  
  \subfloat[Testbeds $4\pm$, $6\pm$ reject anonymous declarations (line 4). %\emph{declaration doesn't declare anything}.
...
\paragraph{Semantic Analysis Failures} We discovered many compiler crashes which can be attributed to improper or missing error handling. Testing compilers under invalid inputs is important.
...
% CLgen result ID 994
Testbed $3\pm$ assertion in \emph{gbe::GenEncoder::handleDouble}
