...
\begin{figure}
  ...
  \caption{Example kernels which crash Intel vectorizer pass.}%
\end{figure}


\begin{figure}
  \centering %
  \subfloat[Testbed $3\pm$ rejects automatic variables with address-space qualifiers.]{%
    TODO
    \label{lst:beignet-automatic-var}
  }\\%
  
  \subfloat[Testbeds $4\pm$, $6\pm$ error \emph{declaration doesn't declare anything}. All other testbeds compile, some with a warning.]{%
...
\begin{figure}
  ...
  \subfloat[Pointer to integer causes Testbeds $3\pm$ to loop indefinitely, consuming more memory, until the entire system memory is used and the process crashes. All other Testbeds compile in < 1s.]{%
...
\cc{Many of the causes can be attributed to improper error handling}
...
Testbed $3\pm$ hangs while assigning a value from a pointer address.
...
\paragraph{Optimizer Hangs} Some compiler crashes are optimization sensitive. Testbeds $1+$ and $2+$ hang on large loop bounds, shown in Figure~\ref{lst:nvidia-opt-loop-hang} \cc{how large does the loop bound have to be?}.
...
\paragraph{Build Failures} Some compilers are more permissive then others. Figure~\ref{lst:intel-decl-doesnt-declare-anything} is rejected by some. Testbeds $1\pm$, $2\pm$, $3\pm$ rejects address space qualifiers on automatic variables.
...
% Testbeds $1\pm$, $2\pm$ reject \texttt{size\_t} kernel arguments (this is right - OpenCL doesn't allow sizet parameters).
...
This is the class of bugs which EMI/CSmith/SPE extensively stress-test.
...
\begin{figure}
  ...
  \vspace{-1em}
  \caption{%
    Comparison of (a) runtimes and (b) test case sizes. DeepSmith test cases are on average evaluated $3.03\times$ faster than CLSmith ($2.45\times$, and $4.46\times$ for generation and execution, respectively), and are two orders of magnitude smaller.
