...
% FIXME: The next setence seems accusatory of Magni, or at the least, scapegoating. Also, performance isn't really disappointing
...
% In Case Study B we have $15$ times fewer programs than in Case Study A, yet the decision space is $3$ times larger, since there are six optimization possibilities rather than two.
...
% We suspect that this approach will be useful for other optimization tasks for which training programs are a scarce resource.

% Our use of transfer learning is just scratching the surface. There are many unanswered questions (e.g. \emph{how many} layers can we transfer? Do we need to re-train once we've initialized, or can we just keep the learned weights from before?). The point to emphasize is that this is a potentially very powerful technique for minimal data modeling, which is unique to DL. It's easy to imagine that with sufficient scale this could be extended well beyond what we demonstrate. E.g. Google can do \emph{zero}-shot translation (i.e. they can do machine translation on a language they've never seen before~\cite{Johnson2016a}). A very promising avenue for further compiler research.
...
% FIXME: The PCA plots are very noisy. i.e., they're not as pretty as I'd hoped. The 'clusters' are not very distinct, and the 'semantically related' tokens contain a few noise words.
...
% The LSTM activations range between $[-1,1]$. The heatmap uses a diverging red-blue color scheme, centered at 0.
...
% \hjl{I'm not sure about this. The neurons in 9e look about as dissimilar as 9f}
% CC: mean variance between activations in 9e: 0.039
% CC: mean variance between activations in 9f: 0.107
...
% For this particular program, the predicted coarsening factors outperform those of Magni \emph{et al.\ } by 84\%. 
% \hjl{So, doesn't predict the best, but gets suspiciously close? Couldn't it just get the best?}
