...
\begin{table*}
	...
	\caption{Classifying test case outcomes of Table~\ref{tab:outcomes} using voting heuristics. Configuration \#. as per Table~\ref{tab:platforms}. $\pm$ denotes optimizations off ($-$) vs on ($+$). The remaining columns denote wrong-code (w), build failure (\textbf{bf}), runtime crash (\textbf{c}), timeout (\textbf{to}), and passed (\textbf{\cmark}) classifications for CLSmith and CLgen, respectively. \cc{Asterisk in 'total' column means the full 48 hours of data have not yet been collected. All results are excluding reductions.}}
	\label{tab:results}
\end{table*}
...
\begin{figure}
	...
	\caption{%
		Runtimes, excluding timeouts. \cc{TODO: CLgen generation times}%
	}%
	\label{fig:runtimes} %
\end{figure}
