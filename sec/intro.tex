...
%Rather than building a custom program generation tool for language X, the idea is instead to use a deep neural network to generate streams of tokens that have a reasonable probability of corresponding to X-programs satisfying the property of interest. The requirements for this approach are:
%a) A large source of example X-programs satisfying the property of interest
%b) An efficient “oracle” that can determine whether the property of interest holds for a
%particular X-program
%c) A neural representation of a program. The simplest is direct encoding the characters of
%the source code, but more efficient approaches exploiting the keywords of the language are possible.
%
...
\begin{itemize}
\item a novel, automatic, and fast approach for the generation of expressive random programs for compiler fuzzing. DeepSmith \emph{infers} programming language syntax, structure, and use from real-world examples not through a manually created probabilistic syntax; % CLSmith requires expert-driven development for every language feature supported. 40+k lines of C++ vs a few hundred lines of Python.
\item through syntactic modeling of handwritten programs, we discover bugs in compilers which the grammar-based approaches cannot, covering more components of the compiler;
% e.g. ill-formed
