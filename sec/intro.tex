...
\begin{itemize}
\item a novel, automatic, and fast approach for the generation of expressive random programs for compiler fuzzing. We \emph{infer} programming language syntax, structure, and use from real-world examples, not through an expert-defined grammar; % CLSmith requires expert-driven development for every language feature supported. 40+k lines of C++ vs a few hundred lines of Python.
...
% e.g. ill-formed
\item we find bugs faster than the state-of-the-art. \cc{I'm not sure that we can back this up. I'm not going to report any CLSmith bugs, so we'll need to find some other metric to make this comparison} In 48 hours we discovered and reported XX bugs in compiler and open source OpenCL implementations, XX\% more than CLSmith over the same time period;
