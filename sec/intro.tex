...
% e.g. ill-formed
\item we find bugs faster than the state-of-the-art. \cc{I'm not sure that we can back this up. CLSmith provides a larger number of ``interesting'' test cases, but there are dupes \& false positives} In 48 hours we discovered and reported XX bugs in compiler and open source OpenCL implementations, XX\% more than CLSmith over the same time period;
