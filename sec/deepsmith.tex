...
\begin{figure}
  \centering %
  \includegraphics[width=\columnwidth]{img/testprocess}%
  \caption{%
...
The last type of undefined behavior comes from division by zero and related mathematical functions which require non-zero values. We apply a simple heuristic -- we change the input values and check to see if the output remains anomalous. While theoretically insufficient, in practice we found that no false positives remained.}
