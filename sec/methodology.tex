...
\begin{table*}[t!]
  ...
  \caption{%
    OpenCL systems, the time spent in automated testing, and the number of bug reports submitted to date. \hl{For each system, two testbeds are created, one with compiler optimizations, the other without.} \cc{refresh bugs submitted}%
  }
  \label{tab:platforms}
\end{table*}
