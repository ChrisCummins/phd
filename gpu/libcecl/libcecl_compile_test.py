...
def test_OpenClCompileAndLinkFlags_smoke_test(
  ...
  proc = clang.Exec(
    ["-x", "c", "-", "-S", "-emit-llvm", "-o", str(bitcode_path)] + cflags,
    stdin=c_program_src,
    stdout=None,
    stderr=None,
  )
  ...
  proc = clang.Exec(
    ["-o", str(bin_path), str(bitcode_path)] + ldflags, stdout=None, stderr=None
  )
  assert not proc.returncode
  ...
  proc = subprocess.Popen(
    [str(bin_path)], env=libcecl_compile.LibCeclExecutableEnvironmentVariables()
  )
  proc.communicate()
