...
def DriveToDataFrame(instances: cldrive_pb2.CldriveInstances,
  ...
  stdout = pbutil.RunProcessMessage(_GetCommand(instances), instances, timeout_seconds=timeout_seconds)
  return pd.read_csv(io.StringIO(stdout.decode('utf-8')), sep=',', quoting=csv.QUOTE_NONE)
