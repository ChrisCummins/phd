...
\newcommand{\fix}[1]{\textcolor{red}{\em#1}}
...
of three \emph{fallback handlers}, \fix{which will iteratively select
...
which the oracle workgroup size is unknown, \fix{meaning that a full
  of exploration of space is not required in order to gather a
...
that it successfully exploits structure in the optimisation
space. \fix{In our evaluation, the largest number of iterations of a
  fallback handler required before selecting a legal workgroup size
  was 2.}
