...
OpenCL programs is a hard task --- OpenCL kernels are sensitive to
properties of the underlying hardware, to the implementation, and even
to the \emph{dataset} that is operated upon. This forces developers to
laboriously hand tune performance on a case-by-case basis, since
simple heuristics fail to exploit the available performance.
...
latency of the repeated memory accessed performed by work-items. As a
...
\begin{figure}
  \centering
  \vspace{-3em}
  \includegraphics[width=1.08\columnwidth]{img/oracle_param_space.pdf}
  \vspace{-2.5em}
  \caption{%
...
size. A minimum of 30 samples were recorded for each combination of
...
shown in Figure~\ref{fig:oracle-wgsize}, which plots the distribution
...
autotuning using regressors to predict kernel runtimes (a) and
speedups (b). Of the two regression techniques, predicting the
...
in this work constitutes the first attempt to autotune the workgroup
...
oracle workgroup size using decision tree classifiers requires the
lowest overhead. This presents a trade-off between classification time
and training time, which could be explored in future work, possibly
using a hybrid combination of the techniques presented in this paper.
