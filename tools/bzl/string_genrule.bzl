...
def _python_string_genrule_impl(ctx):
  ...
  ctx.actions.run_shell(
    command="echo 'STRING = \"\"\"\\' > {out_file}; cat '{in_file}' >> '{out_file}'; echo '\"\"\"' >> {out_file}".format(
      in_file=in_file.path, out_file=out_file.path
    ),
    inputs=[in_file],
    outputs=[out_file],
  ...
  return [
    DefaultInfo(
      files=depset([out_file]), runfiles=ctx.runfiles(files=[out_file])
    ),
    PyInfo(transitive_sources=depset([out_file])),
  ]


python_string_genrule = rule(
  implementation=_python_string_genrule_impl,
