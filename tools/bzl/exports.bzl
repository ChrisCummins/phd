...
exports_repo = rule(
  attrs={
    "github_repo": attr.string(
      mandatory=True, doc="Name of the GitHub repo to export to",
    ),
    "targets": attr.string_list(
      mandatory=True, doc="bazel queries to be find targets to export",
    ),
    "extra_files": attr.string_list(
      default=[], doc="additional files to export",
    ),
    "move_file_mapping": attr.string_dict(
      default={}, doc="files to move on export",
    ),
    "_export_source_tree": attr.label(
      executable=False,
      cfg="host",
      allow_files=True,
