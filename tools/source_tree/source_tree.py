...
def GetCommitsInOrder(
  repo: git.Repo, head_ref: str = "HEAD", tail_ref: typing.Optional[str] = None
) -> typing.List[git.Commit]:
...
def MaybeExportCommitSubset(
  commit: git.Commit, repo: git.Repo, files_of_interest: typing.Set[str]
) -> typing.Optional[git.Commit]:
...
def MaybeGetHexShaOfLastExportedCommit(
  repo: git.Repo, head_ref: str = "HEAD"
) -> typing.List[str]:
...
def ExportCommitsThatTouchFiles(
  commits_in_order: typing.List[git.Commit],
  ...
  for i, commit in enumerate(commits_in_order):
    app.Log(
      1,
      "Processing commit %s of %s (%.2f%%) %s",
      humanize.Commas(i + 1),
      total_commit_count,
      ((i + 1) / len(commits_in_order)) * 100,
      commit,
    )
...
def ExportGitHistoryForFiles(
  ...
  with TemporaryGitRemote(destination, source.working_tree_dir) as remote:
    ...
    commits_in_order = GetCommitsInOrder(
      source, head_ref=head_ref, tail_ref=tail
    )
    ...
    app.Log(
      1,
      "Exporting history from %s commits",
      humanize.Commas(len(commits_in_order)),
    )
    return ExportCommitsThatTouchFiles(
