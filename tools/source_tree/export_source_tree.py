...
flags.DEFINE_string('github_repo', None,
                    'Name of a GitHub repo to export to.')
...
def MaybeTargetToPath(
    ...
    source_root: pathlib.Path) -> typing.Optional[pathlib.Path]:
...
def GetDependentFilesOrDie(
  ...
  with fs.chdir(source_root):
    ...
    grep = subprocess.Popen(['grep', '^/'], stdout=subprocess.PIPE,
                            stdin=bazel.stdout, universal_newlines=True)
...
def GetBuildFilesOrDie(
  ...
  with fs.chdir(repo_root):
    bazel = subprocess.Popen(['bazel', 'query', f'buildfiles(deps({target}))'],
                             stdout=subprocess.PIPE)
    cut = subprocess.Popen(['cut', '-f1', '-d:'], stdout=subprocess.PIPE,
                           stdin=bazel.stdout)
    grep = subprocess.Popen(['grep', '^/'], stdout=subprocess.PIPE,
                            stdin=cut.stdout, universal_newlines=True)
...
def FilterExcludedPaths(paths: typing.List[pathlib.Path],
                        repo_root: pathlib.Path) -> typing.Iterable[
  pathlib.Path]:
  for path in paths:
...
def GetPythonRequirementsForTargetOrDie(
  ...
  with fs.chdir(source_root):
    bazel = subprocess.Popen(['bazel', 'query', f'deps({target})'],
                             stdout=subprocess.PIPE)
    grep = subprocess.Popen(['grep', '^@pypi__'], stdout=subprocess.PIPE,
                            stdin=bazel.stdout, universal_newlines=True)
...
def GetOrCreateRepoOrDie(github: github_lib.Github) -> github_lib.Repository:
  ...
  try:
    ...
    github.get_user().create_repo(
        repo_name, description='PhD repo subtree export',
        homepage='https://github.com/ChrisCummins/phd', has_wiki=False,
        has_issues=False, private=FLAGS.github_repo_create_private)
...
def CommitAndPushOrDie(local: pathlib.Path, remote: github_lib.Repository):
  ...
  with fs.chdir(local):
    ...
    subprocess.check_call(['git', 'commit', '-m',
                           f'Subtree export from {parent_hash}'])
    subprocess.check_call(['git', 'tag', '-a', tag_name, '-m',
                           f'Subtree export from {parent_hash}'])
