...
class TestTargetResult(Base, sqlutil.TablenameFromCamelCapsClassNameMixin):
  ...
  __table_args__ = (
    ...
    sql.UniqueConstraint(
      "invocation_datetime", "bazel_target", name="unique_target_per_invocation"
    ),
  )
...
def GetTestDelta(
  ...
  to_return=[TestTargetResult.bazel_target],
) -> TestDelta:
  """Get the test delta."""
  failed = (
    session.query(*to_return)
  ...
  host, git_branch = (
    session.query(TestTargetResult.host, TestTargetResult.git_branch)
    .filter(TestTargetResult.invocation_datetime == invocation_datetime)
    .first()
  )
  ...
  # Get the last run.
  previous_invocation = (
    session.query(sql.func.max_(TestTargetResult.invocation_datetime))
  ...
  if previous_invocation:
    ...
    return TestDelta(
      broken=failed.filter(
        ~TestTargetResult.bazel_target.in_(
    ...
    return TestDelta(
      broken=passed.all(), fixed=failed.all(), still_broken=[], still_passing=[]
    )
