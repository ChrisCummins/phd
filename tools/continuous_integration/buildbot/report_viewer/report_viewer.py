...
app.DEFINE_string(
  "buildbot_url", "http://localhost:8010/#/builders", "URL of buildbot server."
)
app.DEFINE_string("hostname", "localhost", "Hostname of this server.")
app.DEFINE_integer(
  "port", portpicker.pick_unused_port(), "The port to launch the server on."
)
app.DEFINE_boolean(
  "debug_flask_server", True, "Launch flask app with debugging enabled."
)
app.DEFINE_string(
  "coverage_dir", "/coverage", "Path of coverage files to server statically."
)
...
def RenderInvocation(host, session, invocation):
  ...
  urls = {
    ...
    "buildbot": FLAGS.buildbot_url,
    "github_commit": (
      "https://github.com/ChrisCummins/phd-priv/commit/"
  ...
  hosts = [
    ...
    for x in session.query(
      sql.func.distinct(db.TestTargetResult.host)
    ).order_by(db.TestTargetResult.host)
  ]
  invocations = [
    x[0]
    for x in session.query(
      sql.func.distinct(db.TestTargetResult.invocation_datetime)
    )
...
def index():
  ...
  with database.Session() as session:
    (host,) = (
      session.query(db.TestTargetResult.host)
      .order_by(db.TestTargetResult.invocation_datetime.desc())
      .limit(1)
      .one()
    )
...
def host_latest(host: str):
  with prof.Profile(f"Render /{host}/latest"):
    ...
    with database.Session() as session:
      (invocation,) = (
        session.query(sql.func.max_(db.TestTargetResult.invocation_datetime))
        .filter(db.TestTargetResult.host == host)
        .one()
      )
