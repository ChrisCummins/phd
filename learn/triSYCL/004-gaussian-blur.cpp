...
//
matrix gaussian_blur(const matrix& input,
  ...
  for (int y = 0; y < static_cast<int>(input.nrows()); y++) {
    for (int x = 0; x < static_cast<int>(input.ncols()); x++) {
      float sum = 0;
      float norm = 0;
      for (int j = -static_cast<int>(radius);
           j <= static_cast<int>(radius); j++) {
        for (int i = -static_cast<int>(radius);
             i <= static_cast<int>(radius); i++) {
          int ly = y + j;
          int lx = x + i;
...
matrix sycl_gaussian_blur(const matrix& input,
...

  {
    ...
    cl::sycl::buffer<float, 2> dev_img(input.data(), cl::sycl::range<2>{
    ...
    cl::sycl::buffer<float, 2> dev_out(output.data(), cl::sycl::range<2>{
              ...
              for (int j = -static_cast<int>(kradius[0]);
                ...
                for (int i = -static_cast<int>(kradius[0]);
                  ...
                  auto wi = static_cast<size_t>((j + i) / 2
                   ...
                   const cl::sycl::id<2> imgi{
                     static_cast<size_t>(ly), static_cast<size_t>(lx)};
...
template<
  matrix Gaussian(const matrix& input,
                  const std::vector<float>& weights,
                  const size_t radius)
  >
