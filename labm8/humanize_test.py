...
def test_DecimalPrefix():
  ...
  assert u'1 µs' == humanize.DecimalPrefix(0.0000013, 's', min_scale=None)
...
def test_PrettyFraction():
  ...
  assert u'½' == humanize.PrettyFraction(0.5)
  # Roundeded integer + fraction
  assert u'6⅔' == humanize.PrettyFraction(20.0 / 3.0)
  # Rounded integer, no fraction
  assert u'2' == humanize.PrettyFraction(2.00001)
  # No rounded integer, no fraction
  assert u'0' == humanize.PrettyFraction(0.001)
  # Round up
  assert u'1' == humanize.PrettyFraction(0.99)
  # No round up, edge case
  assert u'⅞' == humanize.PrettyFraction(0.9)
  # Negative fraction
  assert u'-⅕' == humanize.PrettyFraction(-0.2)
  # Negative close to zero (should not be -0)
  assert u'0' == humanize.PrettyFraction(-0.001)
  # Smallest fraction that should round down.
  assert u'0' == humanize.PrettyFraction(1.0 / 16.0)
  # Largest fraction should round up.
  assert u'1' == humanize.PrettyFraction(15.0 / 16.0)
  # Integer zero.
  assert u'0' == humanize.PrettyFraction(0)
  # Check that division yields fraction
  assert u'⅘' == humanize.PrettyFraction(4.0 / 5.0)
  # Custom spacer.
  assert u'2 ½' == humanize.PrettyFraction(2.5, spacer=' ')
