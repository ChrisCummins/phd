...
\begin{abstract}
  ...
  from tuning low level optimisation parameters. Autotuning and
  machine learning offer promising performance benefits in these
  situations, but the high cost of training and lack of standardised
  tooling limits the practicality of autotuning for real world
  programming.
  ...
  for quickly extending software for machine learning-enabled
  autotuning, creating a unified environment training data is shared
  across a network of cooperating devices, and autotuning is performed
  on-the-fly.

  We demonstrate the practicality of OmniTune by targeting the SkelCL
  library. By targeting the workgroup size of OpenCL Stencil skeleton
  kernels, we show that that static tuning can achieve only $26\%$ of
  the optimal performance, while OmniTune achieves $94\%$ of this
  maximum.
\end{abstract}
...
taken to increase the accessibility of such parallelism, this will
only serve to widen the gap between available and utilised performance
as the core counts of hardware continues to increase.
...
parallel programming, allowing users to structure their problem
solving logic sequentially, while offloading the cognitive cost of
parallel coordination to the skeleton author. The rising number of
skeleton frameworks supporting graphics hardware illustrates the
demand for high level abstractions for GPGPU
...
parameter values, and in selecting values to maximise performance,
often one size does not fit all. The suitability of parameter values
depends on the underlying hardware, the program being executed, and
even to the \emph{dataset} that is operated upon. This is especially
...
understandably so: there is a definite appeal to the idea of software
which is capable of maximising its own performance without the need
for human intervention. The unfortunate reality is that while existing
autotuning systems share the unified goal of improving the performance
of their respective optimisation targets, the range of competing
approaches and implementations has resulted in a fragmented state in
which no one system has been able to gain the critical mass to achieve
mainstream traction.

In this work we tackle the issue by designing a unified interface for
autotuning, reducing the amount of redundant and overlapping work
...
The ambition of this work is to demonstrate that, using machine
...
\item \textsc{Push}$(\bf{x}, \bf{p}, \bf{y})$\\*Asynchronously submit
  training point data, as three lists of explanatory variables
  $\bf{x}$, parameter values $\bf{p}$, and observed outcomes $\bf{y}$.
\item \textsc{Pull}$() \to (\bf{x}, \bf{p}, \bf{y})$\\*Request
  training data as three lists of explanatory variables $\bf{x}$,
