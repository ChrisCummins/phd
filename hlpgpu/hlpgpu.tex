...
% From the legal workgroup sizes for a given scenario, we can calculate
% the maximum speedup.
...
% %     \midrule
% %     AMD Tahiti 7970 &   $48\times 4$ &      1.00 &       0.54 &        0.91 \\
% %     Intel i5-2430M &  $64\times 16$ &      0.80 &       0.37 &        0.91 \\
% %     Intel i5-4570 &   $88\times 8$ &     0.89 &       0.33 &        0.89 \\
% %     Intel i7-3820 &  $40\times 24$ &     0.95 &       0.76 &        0.97 \\
% %     NVIDIA GTX 590 &  $12\times 2$ &      0.80 &        0.20 &         0.90 \\
% %     NVIDIA GTX 690 &   $64\times 4$ &     0.93 &       0.32 &        0.84 \\
% %     NVIDIA GTX TITAN &   $64\times 4$ &      1.00 &       0.26 &        0.81 \\
...
To evaluate the performance of the OmniTune SkelCL extension, we
partition the experimental data into \emph{training} and \emph{test}
sets. The training set is used to build the machine learning model for
autotuning. The predicted workgroup size for each entry in the test
set is then used to evaluate the autotuning performance. We use 5
different approaches to partitioning the test and training data, which
each test different aspects of the system. The first is a 10-fold
cross-validation, a standard machine learning model validation
technique in which the the set of all data is shuffled and then
divided into 10 equally sized validation sets. Each validation set is
used to test a model trained on the remaining 90\% of the data. The
...
from $n-1$ devices, and tested against data from the $n^{th}$.

Table~\ref{tab:class} summarises the results of evaluating the
autotuner using each of the different validation
techniques. Autotuning achieves good performance. Average speedups
over the baseline across all validation sets range between
$4.79\times$ and $5.65\times$. The measured overhead of this
autotuning is 2.5ms. Of this, 2.2ms is required for the inter-process
round trip between the OmniTune server and client, and feature
extraction. The remaining 0.3ms is for classification, which is a
lightweight process using decision trees (they can be implemented
using a chain of \texttt{if}/\texttt{else} statements). By isolating
the test cases where illegal or refused parameters were predicted, we
find that the speedup of workgroup sizes across all validation sets to
be $5.26\times$, indicating that the nearest neighbour approach
successfully exploits structure in underlying optimisation spaces.
...
Figure~\ref{fig:class-hmaps} visualises the differences in
classification results between four classifiers: J48, NaiveBayes,
RandomForest, and SMO~\cite{Han2011}. While the average performances
of each of the classifiers is comparable, the distribution of
...
optimal. low cost switch between machine learning algorithms, future
work to meta-tune
...
\end{subfigure}
\caption{%
  Heatmaps of workgroup size predictions for a subset of the
  optimisation space using four different classifiers. The shading in
  each cells indicates if it is predicted less frequently (blue), ore
  more frequently (red) than it is optimal. Colour gradients are
  normalised across plots.%
}
...
\item The performance of a workgroup size for a particular scenario
  ...
  optimal workgroup size is unique for 31.5\% of scenarios.
...
\item The best performance that can be achieved using static tuning is
  optimal for only 15\% of scenarios.
