...
app.DEFINE_string(
  "bytecode_split_type", "all", "The name of the bytecode dataset split to use."
)
app.DEFINE_integer(
  "max_bytecode_split_size",
  0,
...
def GetTrainValTestGroups(
  ...
  with db.Session() as s:
    ...
    num_bytecodes = (
      s.query(sql.func.count(bytecode_database.LlvmBytecode.id))
    ...
    q = (
      s.query(bytecode_database.LlvmBytecode.id)
      .filter(bytecode_database.LlvmBytecode.clang_returncode == 0)
      .filter(~bytecode_database.LlvmBytecode.source_name.like("poj-104:%"))
      .order_by(db.Random())
    )
...
def GetPoj104BytecodeGroups(
  ...
  def GetBytecodeIds(filter_cb) -> typing.List[int]:
    ...
    with db.Session() as session:
      q = (
        session.query(bytecode_database.LlvmBytecode.id)
        .filter(bytecode_database.LlvmBytecode.clang_returncode == 0)
        .filter(filter_cb())
      )
...
def GetPact17BytecodeGroups(
  ...
  with db.Session() as session:
    ...
    all_ids = np.array([r[0] for r in query], dtype=np.int32)
  ...
  if FLAGS.max_bytecode_split_size:
    for group in groups:
      if len(groups[group]) > FLAGS.max_bytecode_split_size:
        app.Log(
          1,
          "Limiting the size of `%s` group to %s elements from %s",
          group,
          humanize.Commas(FLAGS.max_bytecode_split_size),
