...
base_path = Path('/users/zfisches/phd/deeplearning/ml4pl/scripts/table5_lstm/kfold/')
...
def generate():
    ...
    for i, c in enumerate(configs):
        ...
        fmtdic = {
            'timelimit': timelimit, 'idx': i, 'log_db': log_db,
            'database': database, 'groups': groups,
            'encoder': mode[0], 'max_len': mode[1]
        }
        ...
        jobstr = template_header.format(**fmtdic) + info + template_command.format(**fmtdic) + '\n'

        data = 'amd_unbalanced' if 'amd_graph' in fmtdic['database'] else 'nvidia_unbalanced'
        filename = f"run_{i:03d}_{data}_{fmtdic['encoder']}_{groups.replace(',','_')}.sh"
