...
app.DEFINE_database(
  "log_db", log_database.Database, None, "The database to prune."
)
app.DEFINE_boolean(
  "prune_runs_with_no_checkpoints",
  False,
  "Delete logs for run IDs with no model checkpoints.",
)
app.DEFINE_boolean(
  "prune_orphans", False, "Delete orphaned child table entries."
)
...
def PruneOrphans(log_db: log_database.Database):
  ...
  with log_db.Session() as session:
    ...
    app.Log(
      1,
      "Found %s orphaned checkpoints to delete",
  ...
  with log_db.Session() as session:
    ...
    app.Log(
      1,
      "Found %s orphaned batch_logs to delete",
