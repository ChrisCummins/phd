...
def test_PruneRunsWithNoCheckpoints(input_db: log_database.Database):
  ...
  with input_db.Session() as session:
    assert session.query(log_database.Parameter.run_id.distinct()).all() == [
      ("a",)
    ]
    assert session.query(
      log_database.ModelCheckpointMeta.run_id.distinct()
    ).all() == [("a",)]
    assert session.query(log_database.BatchLogMeta.run_id.distinct()).all() == [
      ("a",)
    ]
