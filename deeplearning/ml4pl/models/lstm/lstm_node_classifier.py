...
app.DEFINE_integer(
  "hidden_size", 64, "The size of hidden layer(s) in the LSTM baselines."
)
...
app.DEFINE_string(
  "bytecode_encoder", "inst2vec", "The encoder to use. One of {llvm,inst2vec}"
)
...
app.DEFINE_database(
  "encoded_bytecode_db",
  encoded_bytecode_database.Database,
  None,
  "Path to the encoded bytecode database to populate.",
)
...
app.DEFINE_integer(
  "max_nodes_in_graph",
  25000,
  "The maximum number of grouped statements to feed through " "the LSTM.",
)
#
...
class LstmNodeClassifierModel(classifier_base.ClassifierBase):
  ...
  def __init__(self, *args, **kwargs):
    ...
    def segment_sum_wrapper(args):
      ...
      max_segment_id = tf.cast(
        tf.math.reduce_max(segment_ids) + 1, dtype=tf.int32
      )
    ...
    selector_vector = keras.Input(
      batch_shape=(FLAGS.batch_size, None, 2), name="selector_vector"
    )
  ...
  def GetEncodedBytecodes(
    ...
    typing.List[np.array], typing.List[np.array], typing.List[np.array]
  ]:
    ...
    bytecode_ids_to_encode = list(
      unique_bytecode_ids - id_to_encoded_sequences.keys()
    )
    if bytecode_ids_to_encode:
      with prof.Profile(
        ...
        (
          ...
          grouping_ids,
          node_masks,
        ) = self.encoder.EncodeBytecodes(bytecode_ids_to_encode)
      ...
      with prof.Profile(
        ...
        with self.encoded_bytecode_db.Session(commit=True) as session:
          for bytecode_id, encoded_sequence, segment_ids, node_mask in zip(
            bytecode_ids_to_encode, encoded_sequences, grouping_ids, node_masks
          ):
            result = session.GetOrAdd(
              encoded_bytecode_database.EncodedBytecode, bytecode_id=bytecode_id
            )
  ...
  def MakeMinibatchIterator(
    ...
    for batch in self.batcher.MakeGraphBatchIterator(
      ...
      split_indices = (
        np.where(batch.graph_nodes_list[:-1] != batch.graph_nodes_list[1:])[0]
        + 1
      )
      ...
      if len(node_masks) != len(all_node_y_per_graph):
        raise OSError(
          f"len(node_masks)={len(node_masks)} != "
