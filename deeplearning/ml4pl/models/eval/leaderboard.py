...
def GetBestEpochStats(session, df) -> typing.Optional[pd.DataFrame]:
  ...
  for run_id, row in df.iterrows():
    ...
    query = query.filter(
      log_database.BatchLogMeta.run_id == run_id,
...
def GetLeaderboard(
  ...
  with log_db.Session() as session:
    ...
    query = session.query(
      log_database.BatchLogMeta.run_id,
      sql.func.sum(log_database.BatchLogMeta.elapsed_time_seconds).label(
        "runtime"
      ),
    ...
    query = session.query(
      ...
      sql.func.count(log_database.ModelCheckpointMeta.run_id).label(
        "checkpoints"
      ),
      sql.func.max(log_database.ModelCheckpointMeta.validation_accuracy).label(
        "val_acc"
      ),
    )
