...
def MakePlaceholders(
  stats: graph_database_stats.GraphTupleDatabaseStats,
  ...
  """
  placeholders = {
    "graph_count": tf.compat.v1.placeholder(tf.int32, [], name="graph_count"),
    "node_count": tf.compat.v1.placeholder(tf.int32, [], name="node_count"),
    "adjacency_lists": [
    ...
    "edge_positions": [
      tf.compat.v1.placeholder(
        dtype=tf.int32, shape=[None], name=f"edge_positions_e{i}"
      )
      for i in range(stats.edge_type_count)
    ],
    "incoming_edge_counts": tf.compat.v1.placeholder(
      tf.float32, [None, stats.edge_type_count], name="incoming_edge_counts"
    ),
    "graph_nodes_list": tf.compat.v1.placeholder(
      tf.int32, [None], name="graph_nodes_list"
    ),
    # Dropouts:
    "graph_state_dropout_keep_prob": tf.compat.v1.placeholder(
    ...
    "output_layer_dropout_keep_prob": tf.compat.v1.placeholder(
      tf.float32, [], name="output_layer_dropout_keep_prob"
    ),
    "is_training": tf.compat.v1.placeholder(
      dtype=tf.bool, shape=[], name="is_training"
    ),
    "learning_rate_multiple": tf.compat.v1.placeholder(
      dtype=tf.float32, shape=[], name="learning_rate_multiple"
    ),
  }
  ...
  if stats.node_labels_dimensionality:
    placeholders["node_y"] = tf.compat.v1.placeholder(
      ...
      [None, stats.node_labels_dimensionality],
      name="node_y",
    )
  ...
  if stats.graph_features_dimensionality:
    placeholders["graph_x"] = tf.compat.v1.placeholder(
      ...
      [None, stats.graph_features_dimensionality],
      name="graph_x",
    )
...
def BatchDictToFeedDict(
  ...
  assert (
    placeholders["learning_rate_multiple"] is not None
  ), "learning_rate_multiple is None"

  return feed_dict


def MakeModularOutputLayer(
  initial_node_state, final_node_state, regression_gate, regression_transform
):
  ...
  with tf.compat.v1.variable_scope("modular_output_layer"):
    with tf.compat.v1.variable_scope("gated_regression"):
      ...
      computed_values = tf.nn.sigmoid(
        regression_gate(gated_input)
