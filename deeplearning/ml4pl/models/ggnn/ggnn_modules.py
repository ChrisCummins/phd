  ...
  """
    ...
    def __init__(self, config, pretrained_embeddings=None):
        ...
        if (
            ...
            config.inst2vec_embeddings
            == ggnn_config.NodeTextEmbeddingType.RANDOM
        ):
  ...
  returns incoming messages per node of shape <N, D+S>"""
    ...
    def forward(self, edge_lists, node_states):
        ...
        for i, edge_list in enumerate(edge_lists):
            ...
            if self.msg_mean_aggregation:
                bincount += edge_targets.bincount(
                    minlength=node_states.size()[0]
                )
...
class GGNNLayer(nn.Module):
    ...
    def forward(self, messages, node_states):
        if self.dropout > 0.0:
            F.dropout_(
                messages, p=self.dropout, training=self.training, inplace=True
            )
        ...
        if self.dropout > 0.0:
            F.dropout_(
                output, p=self.dropout, training=self.training, inplace=True
            )
        return output
