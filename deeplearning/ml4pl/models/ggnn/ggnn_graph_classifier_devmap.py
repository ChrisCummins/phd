...
app.DEFINE_float("edge_weight_dropout_keep_prob", 0.8,
...
class GgnnGraphClassifierModel(classifier_base.ClassifierBase):
  ...
  def MakeLossAndAccuracyAndPredictionOps(
    ...
    # TODO(zach) this is still specific to the distribution of the graph_x features in devmap.
    # auxiliary inputs wgsize and dsize
    graph_x = tf.log(self.placeholders["graph_x"])
    graph_x = tf.clip_by_value(graph_x, -1.0, float('inf'))
    ...
    x = tf.layers.dropout(x, rate=1-self.placeholders["out_layer_dropout_keep_prob"],
            training=self.placeholders['is_training'])
    computed_values = tf.layers.dense(x, 2) #, activation=tf.nn.sigmoid) #, activation='sigmoid')
    ...
    graph_loss = tf.losses.softmax_cross_entropy(self.placeholders["graph_y"],
                                           computed_values)
    loss = tf.losses.softmax_cross_entropy(self.placeholders["graph_y"],
                                           computed_values)

    final_loss = loss + FLAGS.intermediate_loss_discount_factor * graph_loss

    return final_loss, accuracies, accuracy, computed_values
