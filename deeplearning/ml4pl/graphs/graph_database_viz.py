...
app.DEFINE_database(
  "graph_db", graph_database.Database, None, "The database to read graphs from."
)
...
def main(argv):
  ...
  with graph_db.Session() as session, multiprocessing.Pool() as pool:
    ...
    ids_to_process += GetIdsInOrderWhere(
      session, lambda: graph_database.GraphMeta.bytecode.in_(FLAGS.bytecode_id)
    )
  ...
  for id_chunk in labtypes.Chunkify(ids_to_process, 512):
    ...
    for input_graph, output_graph in pool.imap_unordered(
      id_chunk, chunksize=64
    ):
    ...
    for input_graph, output_graph in enumerate(input_output_graphs):
      ...
      fs.Write(
        input_graph_dot, graph_viz.GraphToDot(input_graph).encode("utf-8")
      )
      fs.Write(
        output_graph_dot, graph_viz.GraphToDot(output_graph).encode("utf-8")
      )
