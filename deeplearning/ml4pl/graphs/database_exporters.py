...
app.DEFINE_integer(
  "batch_size",
  32,
  "The number of bytecodes to process in-memory before writing" "to database.",
)
app.DEFINE_integer(
  "seed",
  0xCEC,
...
class DatabaseExporterBase(object):
  ...
  def GetProcessInputs(
    ...
    [sqlutil.Database, typing.List[int]], typing.Optional[typing.Any]
  ]:
...
class BytecodeDatabaseExporterBase(DatabaseExporterBase):
  ...
  def Export(
    ...
    if len(output_dbs) != 1:
      raise TypeError(
        "Bytecode exporters only support a single output " "database."
      )
    ...
    for group, bytecode_ids in group_to_ids_map.items():
      ...
      exported_graph_count = self.ExportGroup(
        input_db, output_dbs[0], group, bytecode_ids, pool, batch_size
      )
      elapsed_time = time.time() - group_start_time
      app.Log(
        1,
        "Exported %s %s graphs from %s bytecodes in %s "
        "(%.2f graphs / second)",
    ...
    app.Log(
      1,
      "Exported %s graphs (%s) in %s (%.2f graphs / second)",
  ...
  def ExportGroup(
    ...
    with output_db.Session() as session:
      query = session.query(graph_database.GraphMeta.bytecode_id).filter(
        graph_database.GraphMeta.bytecode_id.in_(bytecode_ids)
      )
      already_done = set([r.bytecode_id for r in query])
      app.Log(
        1,
        "Skipping %s previously-exported bytecodes",
    ...
    jobs = [
      (job_processor, input_db.url, bytecode_ids_chunk)
      for bytecode_ids_chunk in bytecode_id_chunks
    ]
    app.Log(
      1,
      "Divided %s %s bytecode chunks into %s jobs",
      humanize.Commas(len(bytecode_ids)),
      group,
      len(jobs),
    )
    ...
    for graph_metas in workers:
      ...
      app.Log(
        ...
        humanize.Commas(len(graph_metas)),
        group,
        len(bytecode_ids),
...
def _BytecodeWorker(packed_args):
  ...
  with prof.Profile(
    lambda t: (
      f"Processed {humanize.Commas(len(bytecode_ids))}"
...
class GraphDatabaseExporterBase(DatabaseExporterBase):
  ...
  def Export(
    ...
    app.Log(
      1,
      "Divided %s bytecode chunks into %s jobs",
      humanize.Commas(len(bytecode_ids)),
      len(jobs),
    )
