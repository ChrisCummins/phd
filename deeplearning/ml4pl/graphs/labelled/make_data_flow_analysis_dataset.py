...
def GetAnnotatedGraphGenerators(
  *analysis_names: typing.Iterable[str],
) -> typing.List[GraphAnnotator]:
...
def GetOutputsFromStrings(outputs: typing.List[str]) -> typing.List[Output]:
  ...
  return [
    Output(annotator=annotator, db=db) for annotator, db in zip(annotators, dbs)
  ]
  ...
  with prof.Profile(
    ...
    if FLAGS.order_by == "random":
      ...
      frequency_table = np.vstack(
        np.unique(all_bytecodes_to_process, return_counts=True)
      ).T
...
class DataFlowAnalysisGraphExporter(database_exporters.DatabaseExporterBase):
  ...
  def Export(
    ...
    with prof.Profile(
      lambda t: (
        f"Read {humanize.Commas(len(input_ids))} input " "bytecode IDs"
      )
    ):
    ...
    app.Log(
      1,
      "Exported %s graphs in %s " "(%.2f graphs / second)",
      humanize.Commas(exported_graph_count),
  ...
  def _Export(
    ...
    bytecode_id_chunks = np.split(
      ...
      list(
        range(
          0,
          len(bytecodes_to_process_by_output[0]),
          max(batch_size // FLAGS.nproc, 1),
        )
      )[1:],
      axis=1,
    )
    jobs = list(
      (input_db.url, annotators, bytecode_ids_chunk)
      for bytecode_ids_chunk in bytecode_id_chunks
    )
    app.Log(
      1,
      "Divided %s bytecodes into %s %s-bytecode jobs",
      humanize.Commas(len(bytecodes_to_process)),
      len(bytecode_id_chunks),
    ...
    for graph_metas_by_output in workers:
      ...
      app.Log(
        1,
        "Created %s graphs at %.2f graphs/sec. %.2f%% of %s bytecodes "
        "processed",
      ...
      for output, graph_metas in zip(self.outputs, graph_metas_by_output):
        if graph_metas:
          with prof.Profile(
            ...
            added_to_database = ResilientAddUnique(
              output.db, graph_metas, output.annotator.name
            )
    ...
    app.Log(
      1,
      "Exported %s graphs from %s input graphs in %s " "(%.2f graphs / second)",
      humanize.Commas(exported_graph_count),
      humanize.Commas(len(bytecodes_to_process)),
      humanize.Duration(elapsed_time),
      exported_graph_count / elapsed_time,
...
def FetchGraphs(
  ...
  with prof.Profile(
    ...
    bytecode_ids_to_fetch: typing.List[int] = list(
      sorted(set(bytecode_ids_to_process[np.nonzero(bytecode_ids_to_process)]))
    )
  ...
  if bytecode_ids_to_fetch:
    with prof.Profile(
      ...
      if len(bytecodes) != len(bytecode_ids_to_fetch):
        raise EnvironmentError(
          f"Requested bytecodes {bytecode_ids_to_fetch} "
          f"but received {[b.id for b in bytecodes]}"
        )
      bytecode_id_to_bytecode: typing.Dict[int, str] = {
        id_: bytecode for id_, bytecode in zip(bytecode_ids_to_fetch, bytecodes)
      }
...
def _Worker(packed_args):
  ...
  for annotator, bytecode_ids in zip(annotators, bytecode_ids_to_process):
    ...
    start_time = time.time()
    annotator_graph_metas = CreateAnnotatedGraphs(
      annotator, graph_metas, bytecodes
    )
    graph_metas_by_output.append(annotator_graph_metas)
    app.Log(
      1,
      "Produced %s %s graphs at bytecodes/sec=%.2f",
      len(annotator_graph_metas),
      annotator.name,
...
def CreateAnnotatedGraphs(
  ...
  for i, graph_meta in enumerate(graph_metas):
    ...
    try:
      ...
      app.Error(
        "Failed to annotate graph for bytecode " "%d: %s (%s:%s:%s() -> %s)",
        graph_meta.bytecode_id,
        e,
        filename,
        line_number,
        function_name,
