...
def MakeAnnotatedGraphs(
  ...
  # Unpack the args generated by MakeAnnotatedGraphsArgsGenerator().
  # Index into the tuple rather than arg unpacking so that we can assign
  # type annotations.
  worker_id: int = packed_args[0]
  analysis: str = packed_args[1]
  graphs: List[unlabelled_graph_database.ProgramGraph] = packed_args[2]
  ctx: progress.ProgressBarContext = packed_args[3]

  ctx.Log(2, "[worker %s] Processing %s graphs", worker_id, len(graphs))
  ...
  for i, program_graph in enumerate(graphs):
    with ctx.Profile(
      3, f"[worker {worker_id}] Processed graph [{i+1}/{len(graphs)}]"
    ):
