      ...
      [
        graph_database.GraphMeta(
          ...
          graph=graph_database.Graph(
            pickled_data=pickle.dumps({"a": 1, "b": 2})
          ),
        )
        for i in range(512)
      ]
...
def _MockProcessBytecodeInputs(
  ...
  return [
    graph_database.GraphMeta(
      ...
      undirected_diameter=0,
      graph=graph_database.Graph(
        pickled_data=pickle.dumps({"id": bytecode_id, "i": i,})
      ),
    )
...
def test_MockBytecodeExporter(
  bytecode_db_512: bytecode_database.Database, graph_db: graph_database.Database
):
...
def _MockProcessGraphInputs(
  graph_db_session: graph_database.Database.Session,
  bytecode_ids: typing.List[int],
):
  """A fake bytecode job creator returns three graphs per input."""
  graph_inputs = graph_db_session.query(graph_database.GraphMeta).filter(
    graph_database.GraphMeta.id.in_(bytecode_ids)
  )
  return [
    graph_database.GraphMeta(
      ...
      undirected_diameter=0,
      graph=graph_database.Graph(
        pickled_data=pickle.dumps({"id": graph_meta.bytecode_id, "i": i,})
      ),
    )
...
def test_MockGraphExporter(
  graph_db_512: bytecode_database.Database, graph_db: graph_database.Database
):
