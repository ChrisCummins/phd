...
class GraphMeta(Base, sqlutil.PluralTablenameFromCamelCapsClassNameMixin):
  ...
  def CreateFromNetworkX(cls, g: nx.MultiDiGraph, **graph_tuple_opts):
    ...
    node_labels_dimensionality = (
      len(graph_tuple.node_y[0]) if graph_tuple.has_node_y else 0
    )
    graph_features_dimensionality = (
      len(graph_tuple.graph_x) if graph_tuple.has_graph_x else 0
    )
    graph_labels_dimensionality = (
      len(graph_tuple.graph_y) if graph_tuple.has_graph_y else 0
    )
  ...
  def CreateWithNetworkXGraph(cls, g: nx.MultiDiGraph):
    ...
    return GraphMeta(
      ...
      node_labels_dimensionality=(
        len(g.nodes[node]["y"]) if "y" in g.nodes[node] else 0
      ),
...
class Database(sqlutil.Database):
  ...
  def DeleteGraphs(self, graph_ids: typing.List[int]) -> None:
    ...
    with self.Session() as session:
      query = session.query(GraphMeta.id).filter(GraphMeta.id.in_(graph_ids))
      ids_to_delete = [row.id for row in query]

    app.Log(1, "Deleting %s graphs", humanize.Commas(len(ids_to_delete)))
    delete = sql.delete(Graph).where(Graph.id.in_(ids_to_delete))
    self.engine.execute(delete)

    delete = sql.delete(GraphMeta).where(GraphMeta.id.in_(ids_to_delete))
    self.engine.execute(delete)
