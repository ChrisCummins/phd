...
class ProGraMLGraphBuilder(object):
  ...
  def MaybeAddSingleEntryBlock(self, g: nx.MultiDiGraph) -> None:
    ...
    if (
      self.only_add_entry_and_exit_blocks_if_required and len(entry_blocks) == 1
    ):
      entry_block = entry_blocks[0][0]
    else:
      entry_block = f"{g.name}_entry"
      g.add_node(
        entry_block, name=entry_block, type="magic", x=self.dictionary["!MAGIC"]
      )
  ...
  def ComposeGraphs(
    self,
    function_graphs: typing.List[nx.MultiDiGraph],
