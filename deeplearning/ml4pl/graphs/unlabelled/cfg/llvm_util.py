...
def ControlFlowGraphFromDotSource(
  ...
  graph = LlvmControlFlowGraph(name=function_name_match.group(1), tag_hook=tag_hook)
  ...
  for i, node in enumerate(dot.get_nodes()):
    ...
    if tag_hook:
      ...
      other_attrs = {}
    graph.add_node(i, **NodeAttributesToBasicBlock(node.get_attributes()), **other_attrs)
