...
# opt.
_DOT_CFG_FUNCTION_NAME_RE = re.compile(r"\"CFG for '(.+)' function\"")
...
def ControlFlowGraphFromDotSource(dot_source: str) -> LlvmControlFlowGraph:
  ...
  # Determine and mark the entry and exit blocks.
  entry_count = 0
  exit_count = 0
  ...
  # Validate the the CFG has a single entry point.
  if entry_count > 1:
    raise ValueError("Graph has multiple entry blocks")
  elif entry_count < 1:
    raise ValueError("Graph has no entry blocks")

  # CFGs may have multiple exit blocks, e.g. unreachables, exceptions, etc.
  if exit_count < 1:
    raise ValueError("Graph has no exit blocks")
