...
import pytest
from deeplearning.ml4pl.graphs.cdfg import \
...
def test_that_root_node_is_connected_to_entry_points(graph1: nx.MultiDiGraph):
  assert 'root' in graph1
  assert graph1.in_degree('root') == 0
  assert len(graph1.out_edges('root')) == 2
  assert list(graph1.neighbors('root')) == ['A_0', 'B_0']
