  ...
  with bytecode_db.Session() as session:
    sq = (
      session.query(
        ...
        sql.func.count(bytecode_database.LlvmBytecode.bytecode_sha1).label(
          "count"
        ),
      )
    ...
    bytecodes_to_process = (
      session.query(sq.c.bytecode_sha1, sq.c.count)
      .filter(sq.c.count > 1)
    ...
    for i, (bytecode_sha1, duplicate_count) in enumerate(bytecodes_to_process):
      app.Log(
        1,
        "Processing %s of %s, %s duplicates",
        i + 1,
        to_process_count,
        duplicate_count,
      )
      ...
      update = (
        sql.update(bytecode_database.LlvmBytecode)
