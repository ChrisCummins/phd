...
from deeplearning.ml4pl.datasets import make_reachability_dataset as dataset
...
def EncodeAndPad(
  srcs: typing.List[str], padded_length: int, atomizer
) -> np.array:
...
def DataFrameToModelData(
  ...
  sequences = EncodeAndPad(
    df["text:successors"], sequence_length, atomizer
  ).astype(np.int32)
...
class LstmReachabilityModel(object):
  def __init__(
    ...
    app.Log(
      1,
      "Training data: x=[%s, %s], y=[15x %s]",
      self.train_x[0].shape,
      self.train_x[1].shape,
    ...
    app.Log(
      1,
      "Validation data: x=[%s, %s], y=[15x %s]",
      self.valid_x[0].shape,
      self.valid_x[1].shape,
    ...
    app.Log(
      1,
      "Unique sequences: %s of %s (%.2f %%)",
      humanize.Commas(num_uniq_seqs),
  ...
  def TrainAndEvaluate(self, num_epochs: int):
    def OnEpochEnd(epoch, logs):
      ...
      app.Log(
        1,
        "Evaluated test set at epoch %s, accuracy: %.2f%%, " "solved: %.2f%%",
        epoch + 1,
        accuracy * 100,
        solved * 100,
      )
...
class ZeroRReachabilityModel(LstmReachabilityModel):
  ...
  def TrainAndEvaluate(self, num_epochs: int):
    ...
    rule = np.ones if avg > 0.5 else np.zeros
...
def main(argv):
  ...
  app.Log(
    1,
    "Filtered test set from %d to %d graphs with %d blocks in each",
    old_len,
    len(test_df),
    FLAGS.num_classes,
  )
