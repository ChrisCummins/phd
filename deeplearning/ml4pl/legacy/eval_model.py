...
def main(argv):
  ...
  model.summary()
  print(
    f"Total training time: {training_time_natural} "
    f"({time_per_epoch_natural} per epoch)."
  )
  print(f"Number of epochs: {num_epochs}.")
  print(f"Training losses: {losses}.")

  training_data = pbutil.FromFile(
    model_dir / "training_data.pbtxt", ml4pl_pb2.ReachabilityDataset()
  )
  testing_data = pbutil.FromFile(
    model_dir / "testing_data.pbtxt", ml4pl_pb2.ReachabilityDataset()
  )
  ...
  print(
    f"Training data: {num_training_graphs_natural} graphs of "
    f"{num_nodes_natural} nodes each."
  )
  print(
    f"Testing data: {num_testing_graphs_natural} graphs of "
    f"{num_nodes_natural} nodes each."
  )

  num_connections_training = sum(
    sum(len(n.child) for n in entry.graph.node) for entry in training_data.entry
  )
  num_connections_testing = sum(
    sum(len(n.child) for n in entry.graph.node) for entry in testing_data.entry
  )
  ...
  seqs = [
    train_model.ControlFlowGraphToSequence(entry.graph) for entry in data.entry
  ]
  num_uniq_seqs = len(set(seqs))
  print(
    "Unique sequences: {} of {} ({:.2f}%)".format(
      humanize.Commas(num_uniq_seqs),
      humanize.Commas(len(seqs)),
  ...
  print(
    "Unique labels: {} of {} ({:.2f}%)".format(
      humanize.Commas(num_uniq_labels),
      humanize.Commas(len(seqs)),
  ...
  print(
    "Accuracy (excluding first class): {:.2%}".format(
