...
def _print_counters() -> None:
  ...
  """
  print('\r\033[Kfiles: new ', files_new_counter, ', modified ',
        files_modified_counter, '. errors ', errors_counter, '. ',
        status_string[0:25], sep='', end='')
  sys.stdout.flush()
...
def _download_opencl_file(github_token: str, repo, url: str,
  ...
  response = json.loads(requests.get(url, headers={
    'Authorization': 'token ' + str(github_token)}).content.decode('utf-8'))
...
def _scrape_github_for_files(db_path: str, github_username: str, github_pw: str,
                             github_token: str, query_terms: List[str],
