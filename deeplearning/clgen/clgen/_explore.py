...
def _seq_stats(sorted_arr: list) -> str:
  ...
  return ('min: {}, med: {}, avg: {}, max: {}'.format(_bigint(sorted_arr[0]), _bigint(median),
                                                      _bigint(avg),
                                                      _bigint(sorted_arr[len(sorted_arr) - 1])))
...
def explore(db_path: str) -> None:
  ...
  stats.append(
    ('Lines of good preprocessed code', _bigint(pp_lc) + ' ({:.0f}%)'.format(ratio_pp_lcs * 100)))
...
def explore_gh(db_path: str) -> None:
  ...
  stats.append(
    ('Number of forked repositories', _bigint(nb_forks) + ' ({:.0f}%)'.format(ratio_forks * 100)))
  ...
  stats.append(('Content files per repository', 'avg: {:.2f}'.format(avg_nb_ocl_files_per_repo)))
