...
def _print_counters() -> None:
  ...
  """
  print('\r\033[Kfiles: new ', files_new_counter, ', modified ', files_modified_counter,
        '. errors ', errors_counter, '. ', status_string[0:25], sep='', end='')
  sys.stdout.flush()
...
def _download_opencl_file(github_token: str, repo, url: str, stack: List[str]) -> str:
  """
  ...
  response = json.loads(
    requests.get(url, headers={'Authorization': 'token ' + str(github_token)}).content.decode(
      'utf-8'))
  ...
  for line in src.split('\n'):
    ...
    if match:
      ...
      if include_url and include_url not in stack:
        include_src = _download_opencl_file(github_token, repo, include_url)
...
def _process_file(g, github_token: str, db, repo, file, download_file_cb) -> bool:
  """
  GitHub file handler.
  ...
  c.execute("INSERT INTO ContentFiles VALUES(?,?)", (url, contents))
  c.execute("INSERT INTO ContentMeta VALUES(?,?,?,?,?)", (url, path, repo_url, sha, size))
...
def _scrape_github_for_files(db_path: str, github_username: str, github_pw: str, github_token: str,
                             query_terms: List[str], file_is_intetesting, download_file_cb):
      ...
      try:
        ...
        for f in tree_iterator:
          if file_is_intetesting(f):
            try:
              _process_file(g, github_token, db, repo, f, download_file_cb)
...
def fetch_repos(db_path: Path, indir: Path, lang: clgen.Language) -> None:
  ...
  for directory in fs.ls(indir, abspaths=True):
    ...
    output = subprocess.check_output(["git", "--git-dir", gitdir, "remote", "-v"],
                                     universal_newlines=True)
    url = output.split("\n")[0].split("\t")[1].split(" ")[0]
    name = fs.basename(directory)

    output = subprocess.check_output(
      f"git --git-dir {gitdir} rev-list --format=format:'%ai' " + f"--max-count=1 $(git --git-dir "
                                                                  f"{gitdir} rev-parse HEAD) | "
                                                                  f"tail -n1", shell=True,
      universal_newlines=True)
    try:
    ...
    output = subprocess.check_output(
      f"find {directory} -type f {name_str} | grep -v '.git/' || true", shell=True,
      universal_newlines=True)
    ...
    for path in files:
      ...
      try:
        ...
        c.execute('INSERT OR IGNORE INTO ContentFiles VALUES(?,?)', (sha, contents))
...
def fetch_github(db_path: str, github_username: str, github_pw: str, github_token: str,
  ...
  if lang == clgen.Language.OPENCL:
    ...
    query_terms = ['opencl', 'cl', 'khronos', 'gpu', 'gpgpu', 'cuda', 'amd', 'nvidia',
                   'heterogeneous']
  ...
  return _scrape_github_for_files(db_path, github_username, github_pw, github_token, query_terms,
                                  file_is_intetesting, download_file_cb)


def inline_fs_headers(path: Path, stack: List[str], lang: clgen.Language = clgen.Language.OPENCL,
  ...
  for line in src.split('\n'):
    ...
    if match:
      ...
      candidates = [x for x in
                    subprocess.check_output(f'find "{escp_topdir}" -type f -name {esc_basename}',
      ...
      if file_to_inline in stack:
        ...
        outlines.append(clgen.format_as_comment(lang, f'[FETCH] ignored_include({line})'))
      elif file_to_inline:
        # Inline the file by recursively expanding its contents:
        outlines.append(clgen.format_as_comment(lang, f'[FETCH] begin_include({line})'))
        inline_src = inline_fs_headers(file_to_inline, stack)
        outlines.append(inline_src)
        outlines.append(clgen.format_as_comment(lang, f'[FETCH] end_include({line})'))
      else:
        # We didn't find anything suitable, so keep the original
        # include:
        outlines.append(clgen.format_as_comment(lang, f'[FETCH] not_found({line})'))
        outlines.append(line)
    else:
      outlines.append(line)
