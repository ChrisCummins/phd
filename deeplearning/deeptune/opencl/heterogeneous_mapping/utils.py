...
def GetAtomizerFromOpenClSources(
  opencl_srcs: typing.Iterator[str],
...
def AddClassificationTargetToDataFrame(
  ...
  cpu_gpu_runtimes = df[
    ["runtime:intel_core_i7_3820", f"runtime:{gpu_name}",]
  ].values
...
def TrainValidationTestSplits(
  ...
  for i, gpu_name in enumerate(["amd_tahiti_7970", "nvidia_gtx_960"]):
    ...
    gpu_train, gpu_val, gpu_test = np.split(
      gpu_df,
      [
        int(train_val_test_ratios[0] * len(gpu_df)),
        int(
          (train_val_test_ratios[0] + train_val_test_ratios[1]) * len(gpu_df)
        ),
      ],
    )

    cpu_train, cpu_val, cpu_test = np.split(
      cpu_df,
      [
        int(train_val_test_ratios[0] * len(cpu_df)),
        int(
          (train_val_test_ratios[0] + train_val_test_ratios[1]) * len(cpu_df)
        ),
      ],
    )
...
def evaluate(
  model: "HeterogemeousMappingModel",
  df: pd.DataFrame,
  atomizer,
  ...
  for split in TrainTestSplitGenerator(df, seed):
    ...
    if predictions_path.is_file():
      ...
      app.Log(
        1,
        "Predicting %d %s mappings for device %s",
        len(split.test_df),
        model.__name__,
        split.gpu_name,
      )
      predictions = model.predict(
        df=split.test_df, platform_name=split.gpu_name, verbose=FLAGS.verbosity
      )
...
def EvaluatePredictions(
  ...
  p_runtimes = [
    ...
    for prections_row, cpu_gpu_runtime_row in zip(predictions, cpu_gpu_runtimes)
  ]
  p_speedup = zero_r_runtimes / p_runtimes

  # sanity check
  assert (
    len(oracle_device_mappings)
    == len(p_speedup)
    == len(predicted_is_correct)
    == len(predictions)
  )
