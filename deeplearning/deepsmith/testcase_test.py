...
def test_Testcase_ToProto():
  ...
  assert proto.profiling_events[
    0
  ].event_start_epoch_ms == labdate.MillisecondsTimestamp(now)
...
def test_Testcase_GetOrAdd(session):
  ...
  assert testcase.profiling_events[
    0
  ].event_start == labdate.DatetimeFromMillisecondsTimestamp(1021312312)
  ...
  assert testcase.profiling_events[
    1
  ].event_start == labdate.DatetimeFromMillisecondsTimestamp(1230812312)
...
def _AddExistingTestcase(session):
  deeplearning.deepsmith.testcase.Testcase.GetOrAdd(
    ...
    deepsmith_pb2.Testcase(
      toolchain="cpp",
      generator=deepsmith_pb2.Generator(
        name="name", opts={"a": "a", "b": "b", "c": "c",},
      ),
      harness=deepsmith_pb2.Harness(
        name="name", opts={"a": "a", "b": "b", "c": "c",},
      ),
      inputs={"src": "void main() {}", "data": "[1,2]", "copt": "-DNDEBUG",},
      invariant_opts={
        "config": "opt",
        "working_dir": "/tmp",
        "units": "nanoseconds",
