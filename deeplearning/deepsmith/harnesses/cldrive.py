...
class CldriveHarness(
  harness.HarnessBase, harness_pb2_grpc.HarnessServiceServicer
):
...
def GetResultOutcome(
  ...
  timeout_ms = (
    int(result.testcase.harness.opts.get("timeout_seconds", 60)) * 1000
  )
  ...
  if result.returncode == 0:
    ...
    result.returncode == 3
    and "Undefined external function" in result.outputs["stderr"]
  ):
    return deepsmith_pb2.Result.BUILD_FAILURE
