...
class DataStore(object):
  ...
  def SubmitTestcases(
    self,
    request: datastore_pb2.SubmitTestcasesRequest,
  ...
  def _AddOneTestcase(
    self, session: db.session_t, proto: deepsmith_pb2.Testcase
  ) -> None:
  ...
  def _BuildTestcaseRequestQuery(self, session, request) -> db.query_t:
    ...
    if testbed_id and not request.include_testcases_with_results:
      q2 = (
        session.query(deeplearning.deepsmith.result.Result.testcase_id)
    ...
    if testbed_id and not request.include_testcases_with_pending_results:
      q2 = (
        session.query(deeplearning.deepsmith.result.PendingResult.testcase_id)
        .join(deeplearning.deepsmith.testcase.Testcase)
        .filter(
  ...
  def GetTestcases(
    self,
    request: datastore_pb2.GetTestcasesRequest,
    response: datastore_pb2.GetTestcasesResponse,
  ) -> None:
    """Request testcases."""
