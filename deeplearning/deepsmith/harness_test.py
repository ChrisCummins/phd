...
def test_Harness_GetOrAdd(session):
  ...
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptSet).count() == 2
  )
  assert session.query(deeplearning.deepsmith.harness.HarnessOpt).count() == 2
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptName).count() == 2
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptValue).count() == 2
  )
...
def test_Harness_duplicates(session):
  ...
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptSet).count() == 0
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptName).count() == 0
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptValue).count() == 0
  )
  ...
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptSet).count() == 4
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptName).count() == 2
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptValue).count() == 3
  )
...
def test_Harness_GetOrAdd_no_opts(session):
  harness = deeplearning.deepsmith.harness.Harness.GetOrAdd(
    session, deepsmith_pb2.Harness(name="name", opts={},)
  )
...
def test_Harness_GetOrAdd_only_different_optset(session):
  harness_a = deeplearning.deepsmith.harness.Harness.GetOrAdd(
    session,
    deepsmith_pb2.Harness(name="name", opts={"a": "A", "b": "B", "c": "C",},),
  )
  harness_b = deeplearning.deepsmith.harness.Harness.GetOrAdd(
    session, deepsmith_pb2.Harness(name="name", opts={"d": "D",},)
  )
  ...
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptName).count() == 4
  )
  assert (
    session.query(deeplearning.deepsmith.harness.HarnessOptValue).count() == 4
  )
...
def test_Harness_GetOrAdd_rollback(session):
  deeplearning.deepsmith.harness.Harness.GetOrAdd(
    session, deepsmith_pb2.Harness(name="name", opts={"a": "1", "b": "2",},)
  )
  assert session.query(deeplearning.deepsmith.harness.Harness).count() == 1
  assert session.query(deeplearning.deepsmith.harness.HarnessOpt).count() == 2
  assert (
