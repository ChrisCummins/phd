...
class Testcase(db.Table):
  ...
  @classmethod
  def GetOrAdd(
    cls, session: db.session_t, proto: deepsmith_pb2.Testcase
  ) -> "Testcase":
...
class TestcaseInputSet(db.Table):
  ...
  # Constraints.
  __table_args__ = (
    sql.PrimaryKeyConstraint("id", "input_id", name="unique_testcase_inputset"),
  )
...
class TestcaseInput(db.Table):
  ...
  @classmethod
  def GetOrAdd(
    cls, session: db.session_t, name: str, value: str
  ) -> "TestcaseInput":
    ...
    return labm8.py.sqlutil.GetOrAdd(
      session,
      TestcaseInput,
      name=TestcaseInputName.GetOrAdd(session, string=name,),
      value=TestcaseInputValue.GetOrAdd(session, string=value,),
    )
...
class TestcaseInvariantOpt(db.Table):
  ...
  def GetOrAdd(
    ...
    return labm8.py.sqlutil.GetOrAdd(
      session,
      cls,
      name=TestcaseInvariantOptName.GetOrAdd(session, string=name,),
      value=TestcaseInvariantOptValue.GetOrAdd(session, string=value,),
    )
