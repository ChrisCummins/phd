...
class ClgenOpenClFilters(difftests.FiltersBase):
  ...
  def PreDifftest(
    ...
    if any(
      result.outcome == deepsmith_pb2.Result.PASS for result in difftest.result
    ):
      if ContainsFloatingPoint(difftest.result[0].testcase):
        app.Log(1, "Cannot difftest OpenCL kernel with floating point.")
        return None
      if HasVectorInputs(difftest.result[0].testcase):
        app.Log(1, "Cannot difftest OpenCL kernel with vector inputs.")
        return None
    return difftest
  ...
  def PostDifftest(
    ...
    for result, outcome in zip(difftest.result, difftest.outcome):
      ...
      if (
        result.outcome == deepsmith_pb2.Result.BUILD_FAILURE
        and LegitimateBuildFailure(result)
      ):
        app.Log(1, "Cannot difftest legitimate build failure.")
        return None
      ...
      if (
        outcome == deepsmith_pb2.DifferentialTest.ANOMALOUS_BUILD_FAILURE
        and result.testbed.opts["opencl_version"] == "1.2"
      ):
        app.Log(1, "Cannot difftest build failures on OpenCL 1.2.")
      ...
      # testcase.
      if (
        outcome == deepsmith_pb2.DifferentialTest.ANOMALOUS_RUNTIME_CRASH
        or outcome == deepsmith_pb2.DifferentialTest.ANOMALOUS_RUNTIME_PASS
        or outcome == deepsmith_pb2.DifferentialTest.ANOMALOUS_WRONG_OUTPUT
        or outcome == deepsmith_pb2.DifferentialTest.ANOMALOUS_RUNTIME_TIMEOUT
      ):
        if RedFlagCompilerWarnings(result):
          app.Log(
            1,
            "Cannot difftest anomalous runtime behaviour with red "
...
def RedFlagCompilerWarnings(result: deepsmith_pb2.Result) -> bool:
  return (
    ...
    or "incompatible pointer to integer conversion"
    or "comparison between pointer and integer"
    or "warning: incompatible"
    or "warning: division by zero is undefined"
    or "warning: comparison of distinct pointer types"
    or "is past the end of the array"
    or "warning: comparison between pointer and"
    or "warning: array index"
    or "warning: implicit conversion from"
    or "array index -1 is before the beginning of the array"
    or "incompatible pointer"
    or "incompatible integer to pointer "
  ) in result.outputs["stderr"]


def LegitimateBuildFailure(result: deepsmith_pb2.Result) -> bool:
  return (
    "use of type 'double' requires cl_khr_fp64 extension"
    or "implicit declaration of function"
    or (
      "function cannot have argument whose type is, or " "contains, type size_t"
    )
    or "unresolved extern function"
    or "error: cannot increment value of type"
    or "subscripted access is not allowed for OpenCL vectors"
    or "Images are not supported on given device"
    or "error: variables in function scope cannot be declared"
    or "error: implicit conversion "
    or "Could not find a definition "
  ) in result.outputs["stderr"]
