...
EXTENDING:
  ...
  * Change the parameters of the network.
...
and either + or - (for OpenCL optimizations on or off, respectively), to
experiment script:

    $ ./02_evaluate_harness/run.sh  <DEVICE_ID>

For example, to run the experiments on the Intel device listed above with
optimizations enabled, I would run $ ./02_evaluate_harness/run.sh 2+.

The script runs XXX testcases, located in the data/testcases directory. You
are free to run it on as many different OpenCL devices as you have
(per-device results will be stored), and you may add additional testcases to
this directory to execute them. For example, you could copy the testcases
generated by your model from the previous step:

    $ cp ./01_evaluate_generator/output/generated_testcases/* \
        ./02_evaluate_harness/data/testcases
    $ ./02_evaluate_harness/run.sh

The results of execution are written to
./02_evaluate_harnesses/output/results/<device_id>/
