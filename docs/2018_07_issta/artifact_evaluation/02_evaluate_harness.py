...
app.DEFINE_boolean(
  "opencl_opt", True, "If --noopencl_opt is set, disable OpenCL optimizations."
)
...
def main(argv):
  ...
  if not len(testcases):
    raise app.UsageError(
      "No testcases found: '%s'", " ".join(input_directories)
    )
  ...
  app.Log(
    1,
    "Executed %d testcases and wrote results to %s",
    len(res.results),
    output_directory,
  )
  execution_times = [
  ...
  app.Log(
    1,
    "Average time to evaluate testcase: %.2f ms",
