...
\begin{abstract}
  The quality of predictive performance tuning is bound by the
  ...
  We present a novel methodology for generating representative compute
  kernels on-demand. Given a corpus of example programs, we apply deep
  learning across the implementation space, learning a language model
  from which we obtain new kernels through a process of rejection
  sampling. We demonstrate our approach for a state-of-the-art machine
  learning OpenCL autotuner. With the addition of synthesized OpenCL
  kernels, we improve the accuracy of learned predictions from XXX\%
...
Predictive modeling is a promising method for building effective
optimization heuristics over large optimization spaces. Whereas
traditionally optimization heuristics are crafted at the expense of
months of developer effort, the optimization space is empirically and
automatically explored to generate a set of training data over which
machine learning is applied. The learned optimizations often
outperform human experts~\cite{Ding2015,Hu2015}.

The challenge of predictive modeling is to be explore a sufficiently
large subset of a given optimization spaces so as to be able to make
meaning predictions, using some finite combination of programs,
datasets, and optimizations. In the program space, this means the
curation and selection of a number of programs to act as
``representative benchmarks''. The curation of programs is time
consuming, and as such, suites of pre-packaged benchmarks are in
widespread use.
...
\begin{quote}
  \emph{RQ1: Does the addition of representative benchmarks aid the
    quality of machine learning predictions?}
\end{quote}
...
\begin{quote}
  \emph{RQ2: Given a program checker and a corpus of example programs,
    can deep learning be used to generate new programs?}
\end{quote}

\noindent
The key contributions of this work are as follows:%
\begin{itemize}
\item a novel methodology for synthesizing compute kernels for machine
  learning-based performance tuning;
\item a large scale source code evaluation and re-writing for
  character-level language modeling using deep learning;
...
We surveyed the benchmarking methodologies of research papers in the
field of GPU performance tuning from top tier conferences between
2013--2016: CGO, HiPC, PACT, and PPoPP. For each of the 27 papers, we
noted the quantity and sources of all kernels they used as
benchmarks\footnote{Anonymized raw data available at:
  \url{http://bit.ly/XXX}}. %
...
used: Rodinia~\cite{Che2009}, the NVIDIA
SDK\footnote{\url{https://developer.nvidia.com/opencl/}},
...
\begin{figure}%[t]
  ...
  % \end{tikzpicture}
  \caption{%
    Number of benchmark kernels used in GPU research papers. The
    median value is 14.%
  }
...
\begin{figure}%[t]
  ...
  \caption{%
    Origins of benchmark kernels used in GPU research papers. The 4
...
\begin{table*}% tab:platforms
  ...
  \begin{tabular}{l c c c c}%C{1.65cm} C{1.65cm} C{1.65cm} C{1.65cm}}
    ...
    & \textbf{Intel i7-3820} & \textbf{AMD Tahiti 7970} & \textbf{Intel x} & \textbf{AMD HD 5450} \\
    \midrule
    \textbf{Frequency} & 3.6 GHz & 1000 MHz & x GHz & 650 MHz \\
    \textbf{\#. Cores} & 4 & 2048 & 8 & 80 \\
    \textbf{Memory} & 8 GB & 3 GB & x & 1024 MB \\
    \textbf{Throughput} & 95 GFLOPS & 3.79 TFLOPS & x GFLOPS & 104 GFLOPS
    \\
    % \midrule
...
Designing workloads~\cite{Eeckhout2002}. Characterization of NAS
...
This is the first application of deep learning for synthesizing
programs.
