...
their application-specific code. This greatly reduces the challenge of
...
\vspace{-1.8em} % Shrink vertical padding
\caption{}
...
hardware. At runtime, once an OpenCL program has been compiled to a
kernel, users can query the maximum workgroup size supported by that
particular kernel dynamically. This value cannot easily be obtained
...
code. Dataset variables include \fix{the data types (input and
...
Equation, and Gaussian Blur\fix{~\cite{Steuwer2011}}. Performance data
...
benchmarks programs using the \emph{baseline} workgroup size
\fix{(defined below)}. The output of real-world benchmarks with other
workgroup sizes is compared to this gold standard output to test for
...
against runtimes for each of three parameters:
%
\begin{itemize}
...
\item \emph{Baseline} --- The \emph{baseline} parameter is the
  ...
  being legal for all scenarios. \fix{Such a baseline value represents the
  \emph{best} possible performance which can be achieved using a
  single, statically chosen workgroup size. By defining
  $W_{safe} \in W$ as the intersection of legal workgroup sizes, the
  baseline $\bar{w}$ can be found using:
  %
  \begin{align}
  ...
  \end{align}
  %
  } For our experimental data, we find this value to be
  $w_{(4 \times 4)}$.
  %
...
% \end{subfigure}
\caption{%
  Heatmaps of autotuner predictions for a subset of the \fix{explored}
  optimization space \fix{($w_c < 80, w_r < 80$)} using two different
...
express). \fix{Autotuning for algorithmic skeletons is performed using
...
for which it has no prior training data. \fix{By performing autotuning
  at the skeletal level, the system is able to exploit underlying
...
multiple cooperating devices, \fix{and targeting multiple parameters
  simultaneously}.
