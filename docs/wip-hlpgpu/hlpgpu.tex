...
quick re-purposing for autotuning targets \fix{also lightweight
  runtime benefits}. OmniTune provides a lightweight interface for
...
\begin{itemize}
\item
  \textsc{Request}$(x : \text{\fix{feature vector}}) \to p :
  \text{\fix{param}}$\\*Given
  explanatory variables $x$, request the parameter values $p$ which
  are expected to provide maximum performance.
\item
  \textsc{RequestTraining}$(x : \text{\fix{feature vector}}) \to p :
  \text{\fix{param}}$\\*Given
  explanatory variables $x$, allow the server to select parameter
  values $p$ for evaluating their fitness.
\item
  \textsc{Submit}$(x : \text{\fix{feature vector}}, p :
  \text{\fix{param}}, y : \text{\fix{fitness}})$\\*Submit
  an observed measurement of fitness $y$ for parameter values $p$,
  given explanatory variables $x$.
\item
  \textsc{Refuse}$(x : \text{\fix{feature vector}}, p :
  \text{\fix{param}})$\\*Refuse
  parameter values $p$, given a set of explanatory variables $x$. Once
  refused, those parameters are blacklisted and will not be returned
  by any subsequent calls to \textsc{Request()} or
...
\begin{itemize}
\item
  \textsc{Push}$(\bf{x} : \text{\fix{feature vectors}}, \bf{p} :
  \text{\fix{params}}, \bf{y} :
  \text{\fix{fitnesses}})$\\*Asynchronously
  submit training data as three lists: explanatory variables $\bf{x}$,
  parameter values $\bf{p}$, and observed outcomes $\bf{y}$.
\item
  \textsc{Pull}$() \to (\bf{x} : \text{\fix{feature vectors}}, \bf{p}
  : \text{\fix{params}}, \bf{y} : \text{\fix{fitnesses}})$\\*Request
...
hardware. Typical values are powers of two, e.g.\ 1024, 4096, 8192
\fix{not always true!}. At runtime, once an OpenCL program has been
compiled to a kernel, users can query the maximum workgroup size
...
\fix{Consider moving this paragraph to evaluation:} The
\emph{baseline} workgroup size $\bar{w}$ is the value which provides
the best average case performance across a set of scenarios. Such a
...
global caches). Program variables include \fix{instruction densities
  for each instruction type}, the total number of basic blocks, and
...
the same stencil codes, identified by a checksum of the source
code. Dataset variables include \fix{a description of the data types
  (input and output), and dimensions of the input matrix and stencil
  region}.
...
Equation, and Gaussian Blur \fix{citations needed}. Performance data
was collected from 7 experimental platforms, comprising 4 GPU devices:
AMD Tahiti 7970, Nvidia GTX 590, Nvidia GTX 690, Nvidia GTX TITAN; and
3 CPU devices: Intel i5-2430M, Intel i5-4570, i7-3820. Each platform
was unloaded, frequency governors disabled, and benchmark processes
set to the highest priority available to the task scheduler. Datasets
and programs were stored in an in-memory file system. For each
program, dataset sizes of size $512\times512$, $1024\times1024$,
...
\caption{%
  ...
  space \fix{($w_c < 80, w_r < 80$)} using two different
...
performance data is lifted from the user \fix{and it helps to be done
  at skeletal level! It wouldn't work as well with unstructured
  code}. The system presented in this paper aims to solve this issue
by providing a generic interface for implementing machine
learning-enabled autotuning. OmniTune is a novel framework for
autotuning which has the benefits of a fast, ``always-on'' interface
for client applications, while being able to synchronize data with
global repositories of knowledge which are built up across devices. To
...
codes. This optimization space is complex, non linear, and critical
...
multiple cooperating devices \fix{and multiple params simultaneously}.
