...
\begin{itemize}
\item
  \textsc{Request}$(x : \text{feature vector}) \to p :
  \text{param}$\\*Given
...
\item
  \textsc{Submit}$(x : \text{feature vector}, p :
  \text{param}, y : \text{fitness})$\\*Submit
  an observed measurement of fitness $y$ for parameter values $p$,
  given explanatory variables $x$.
\item
  \textsc{Refuse}$(x : \text{feature vector}, p :
  \text{param})$\\*Refuse
...
\item
  ...
  : \text{params}, \bf{y} : \text{fitnesses})$\\*Request
...
include the data types (input and output), and dimensions of the input
matrix and stencil region.
...
\item \emph{Baseline} --- The \emph{baseline} parameter is the
  ...
  being legal for all scenarios. Such a baseline value represents the
